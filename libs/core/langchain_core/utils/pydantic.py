...
def _create_subset_model_v2(
    ...
    *,
    ...
    # Get type hints properly to preserve all annotation information
    # This is compatible with Pydantic 2.14+ which uses getattr instead of __dict__.get
    try:
        model_hints = get_type_hints(model, include_extras=True)
    except Exception:
        # Fallback for edge cases where get_type_hints fails
        model_hints = getattr(model, "__annotations__", {}).copy()

    for field_name in field_names:
        field = model.model_fields[field_name]
        description = descriptions_.get(field_name, field.description)

        # Get the proper annotation from type hints
        annotation = model_hints.get(field_name, field.annotation)
    ...
    # Let Pydantic handle __annotations__ setup internally
    # Avoid direct mutation which violates Python's invariant:
    # getattr(cls, '__annotations__', {}) should equal
    # cls.__dict__.get('__annotations__', {})
    # See: https://github.com/langchain-ai/langchain/issues/37835
    rtn = cast(
        "type[BaseModel]",
