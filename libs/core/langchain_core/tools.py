...
from typing import Any, Awaitable, Callable, Dict, List, Optional, Tuple, Type, Union, Annotated, get_args
...
class InjectedToolArg:
    """Marker class for tool arguments that should be injected at runtime.

    Use this type annotation to mark function parameters that should not be
    included in the tool's JSON schema (hidden from the chat model).
    These parameters will be injected at runtime rather than being provided
    by the model.
    ...
    Example:
        ...
        # Only 'input_data' will appear in the schema, not 'user_id'
    """
    pass


def _check_injected_arg_type(type_: type) -> bool:
    """Check if a type annotation indicates an injected tool argument.

    Args:
        type_: The type annotation to check

    Returns:
        True if the type is InjectedToolArg or contains InjectedToolArg
    """
    if type_ is InjectedToolArg:
        return True

    # Check if it's a generic type containing InjectedToolArg
    # e.g., Optional[InjectedToolArg], Union[InjectedToolArg, str], etc.
...
def _get_filtered_args(
    ...
    """Get the arguments from a function's signature, excluding injected args."""
    schema = inferred_model.schema()["properties"]
    valid_keys = signature(func).parameters
    sig = signature(func)

    # Filter out run_manager, callbacks, and injected arguments
    filtered = {}
    for k in valid_keys:
        if k in ("run_manager", "callbacks"):
            continue

        # Check if the parameter has an InjectedToolArg type annotation
        param = sig.parameters[k]
        if param.annotation != inspect.Parameter.empty:
            if _check_injected_arg_type(param.annotation):
                continue
...
def create_schema_from_function(
    ...
    Returns:
        ...
        excluding injected arguments (marked with InjectedToolArg)
    """
    # https://docs.pydantic.dev/latest/usage/validation_decorator/
    validated = validate_arguments(func, config=_SchemaConfig)  # type: ignore
    inferred_model = validated.model  # type: ignore

    # Remove special parameters that shouldn't be in the schema
    for field_name in ["run_manager", "callbacks"]:
        if field_name in inferred_model.__fields__:
            del inferred_model.__fields__[field_name]

    # Also remove fields with InjectedToolArg type annotation
    sig = signature(func)
    for param_name, param in sig.parameters.items():
        if param.annotation != inspect.Parameter.empty:
            if _check_injected_arg_type(param.annotation):
                if param_name in inferred_model.__fields__:
                    del inferred_model.__fields__[param_name]
