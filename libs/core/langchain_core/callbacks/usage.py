...
class UsageMetadataCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        if usage_metadata:
            target_model = model_name or "unknown"
            with self._lock:
                if target_model not in self.usage_metadata:
                    self.usage_metadata[target_model] = usage_metadata
                else:
                    self.usage_metadata[target_model] = add_usage(
                        self.usage_metadata[target_model], usage_metadata
                    )
