    ...
    if block["type"] == "image":
        ...
        if "url" in block:
            ...
            if block.get("mime_type"):
                video_url["mime_type"] = block["mime_type"]
            formatted_block = {
                "type": "video_url",
                "video_url": video_url,
            }
            ...
            if "mime_type" not in block:
                error_msg = "mime_type key is required for base64 data."
                raise ValueError(error_msg)
            base64_data = block["data"] if "data" in block else block["base64"]
            formatted_block = {
                "type": "video_url",
                "video_url": {
                    "url": f"data:{block['mime_type']};base64,{base64_data}",
                    "mime_type": block["mime_type"],
                },
            }
        elif block.get("source_type") == "id" or "file_id" in block:
            file_id = block["id"] if "source_type" in block else block["file_id"]
            video_url = {"url": file_id}
            if block.get("mime_type"):
                video_url["mime_type"] = block["mime_type"]
            formatted_block = {
                "type": "video_url",
            ...
            error_msg = "Keys url, base64, or file_id required for video blocks."
            raise ValueError(error_msg)
...
def _convert_openai_format_to_data_block(
    ...
    if block.get("type") == "video_url":
        video_data = block.get("video_url", {})

        known_keys = {"type", "video_url"}
        extras = _extract_extras(block, known_keys)

        video_known_keys = {"url", "mime_type"}
        video_extras = _extract_extras(video_data, video_known_keys)

        all_extras = {**extras}
        for key, value in video_extras.items():
            all_extras[f"video_{key}"] = value
