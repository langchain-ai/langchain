...
def test_chat_invalid_input_variables_extra() -> None:
    ...
    with pytest.raises(ValueError):
        ChatPromptTemplate(
            messages=messages, input_variables=["foo"], validate_template=True  # type: ignore[arg-type]
        )
    assert (
...
def test_chat_invalid_input_variables_missing() -> None:
    ...
    with pytest.raises(ValueError):
        ChatPromptTemplate(
            messages=messages, input_variables=[], validate_template=True  # type: ignore[arg-type]
        )
    assert ChatPromptTemplate(
        messages=messages, input_variables=[]  # type: ignore[arg-type]
    ).input_variables == ["foo"]
...
def test_chat_valid_infer_variables() -> None:
    ...
    prompt = ChatPromptTemplate(  # type: ignore[call-arg]
        messages=messages, partial_variables={"formatins": "some structure"}  # type: ignore[arg-type]
    )
    assert set(prompt.input_variables) == {"question", "context"}
    assert prompt.partial_variables == {"formatins": "some structure"}
