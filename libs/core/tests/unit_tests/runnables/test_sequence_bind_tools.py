...
Tests the fix for issue #28848: ChatOpenAI bind_tools not callable after with_structured_output
"""
...
def test_sequence_bind_tools_structured_output_pattern():
...
def test_sequence_bind_tools_with_kwargs():
...
def test_sequence_bind_tools_sequence_with_middle_steps():
    """Test bind_tools with longer sequences."""
    mock_model = RunnableLambda(lambda x: x)
    mock_model.bind_tools = lambda tools, **kwargs: RunnableLambda(lambda x: "bound")
...
def test_sequence_bind_tools_preserves_name():
    ...
    sequence = RunnableSequence(mock_model, mock_parser, name="my_structured_llm")
...
def test_sequence_bind_tools_no_bindable_step_raises_error():
...
def test_sequence_bind_tools_multiple_tools():
    """Test binding multiple tools at once."""
    mock_model = RunnableLambda(lambda x: x)
    
    def bind_tools_method(tools, **kwargs):
        return RunnableLambda(lambda x: f"bound_with_{len(tools)}_tools")
