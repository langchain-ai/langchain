...
def test_local_cache_sync() -> None:
    ...
    chat_model = FakeListChatModel(cache=local_cache, responses=[
        "hello", "goodbye"
    ])
...
def test_global_cache_sync() -> None:
    ...
    global_cache = InMemoryCache()
    set_llm_cache(global_cache)
    chat_model = FakeListChatModel(cache=True, responses=[
        "hello", "goodbye", "meow", "woof"
    ])
    assert (chat_model.invoke("How are you?")).content == "hello"
    # If the cache works we should get the same response since
    # the prompt is the same
...
def test_no_cache_sync() -> None:
    global_cache = InMemoryCache()
    set_llm_cache(global_cache)
    chat_model = FakeListChatModel(cache=False, responses=['hello', 'goodbye'])  # Set cache=False
    assert (chat_model.invoke("How are you?")).content == "hello"
...
def test_global_cache_stream() -> None:
    ...
    global_cache = InMemoryCache()
    set_llm_cache(global_cache)
    messages = [AIMessage(content='hello world'), AIMessage(content='goodbye world')]
