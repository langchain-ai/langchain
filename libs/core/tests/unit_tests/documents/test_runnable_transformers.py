...
def test_transform(cls: Type) -> None:
    doc1 = langchain_core.documents.Document(page_content="my test")
    doc2 = langchain_core.documents.Document(page_content="other test")
    r = cls().transform_documents(documents=[doc1, doc2])
    assert r == [
        langchain_core.documents.Document(page_content=doc1.page_content.upper()),
        langchain_core.documents.Document(page_content=doc2.page_content.upper()),
    ]
...
def test_invoke_pipeline(cls: Tuple[Type, Type]) -> None:
    ...
    assert r == [
        langchain_core.documents.Document(page_content=doc1.page_content.lower()),
        langchain_core.documents.Document(page_content=doc2.page_content.lower()),
    ]
