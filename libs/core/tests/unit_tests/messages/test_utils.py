...
def test_convert_to_openai_messages_does_not_mutate_file_block() -> None:
    file_block = {
        "type": "file",
        "file": {"file_data": "data:application/pdf;base64,QQ=="},
    }

    result = convert_to_openai_messages([HumanMessage(content=[file_block])])

    assert file_block == {
        "type": "file",
        "file": {"file_data": "data:application/pdf;base64,QQ=="},
    }
    assert result[0]["content"][0]["file"]["filename"] == "LC_AUTOGENERATED"
