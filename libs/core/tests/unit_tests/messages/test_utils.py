...
def test_convert_to_openai_messages_file_block_no_filename_does_not_mutate() -> None:
    """convert_to_openai_messages must not modify the caller's message in place.

    When an OpenAI file block omits 'filename', the function injects the
    'LC_AUTOGENERATED' sentinel.  Prior to the fix this was written directly
    into the caller's dict, so any subsequent use of the same message (e.g.
    sending to a second provider) would carry the spurious key forward.
    ...
    # Run the conversion — this must not raise and must produce the right output.
    result = convert_to_openai_messages([message])

    # The output should carry the fallback filename …
    output_block = result[0]["content"][0]  # type: ignore[index]
    assert output_block["file"]["filename"] == "LC_AUTOGENERATED"

    # … but the caller's original block must be completely untouched.
    assert "filename" not in file_block["file"], (
        "convert_to_openai_messages must not mutate the caller's block dict"
    )


def test_convert_to_openai_messages_file_block_with_filename_unchanged() -> None:
    """A file block that already carries 'filename' must be passed through as-is."""
    file_block: dict = {
        "type": "file",
        "file": {
            "filename": "report.pdf",
    ...
    output_block = result[0]["content"][0]  # type: ignore[index]
    assert output_block["file"]["filename"] == "report.pdf"
