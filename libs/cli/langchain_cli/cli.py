...
app.add_typer(template_namespace.package_cli, name=TEMPLATE_CMD, help=template_namespace.__doc__)
...
def serve(
    *,
    port: Annotated[Optional[int], typer.Option(help="The port to run the server on")] = None,
    host: Annotated[Optional[str], typer.Option(help="The host to run the server on")] = None,
) -> None:
    try:
        ...
        typer.secho(f"Error: {e}. This project does not seem to be a template.", fg=typer.colors.RED)
        app_namespace.serve(port=port, host=host)
    except FileNotFoundError:
        typer.secho("Error: `pyproject.toml` not found. Ensure you are in a valid project directory.", fg=typer.colors.RED)
    else:
