...
class SandboxIntegrationTests(BaseStandardTests):
    ...
    def test_download_error_is_directory(
        ...
        sandbox_backend.execute(f"rm -rf {_quote(dir_path)} && mkdir -p {_quote(dir_path)}")  # nosemgrep: sqlalchemy-execute-raw-query,formatted-sql-query  # not SQLAlchemy/SQL; shell path is sanitized via shlex.quote
    ...
    def test_upload_missing_parent_dir_or_roundtrip(
        ...
        content = b"nope"
        sandbox_backend.execute(f"rm -rf {_quote(dir_path)}")  # nosemgrep: sqlalchemy-execute-raw-query,formatted-sql-query  # not SQLAlchemy/SQL; shell path is sanitized via shlex.quote
    ...
    def test_grep_across_nested_directories(
        ...
        sandbox_backend.execute(f"mkdir -p {_quote(base_dir + '/sub1/sub2')}")  # nosemgrep: sqlalchemy-execute-raw-query,formatted-sql-query  # not SQLAlchemy/SQL; shell path is sanitized via shlex.quote
    ...
    def test_grep_with_globstar_include_pattern(
        ...
        sandbox_backend.execute(f"mkdir -p {_quote(base_dir + '/a/b')}")  # nosemgrep: sqlalchemy-execute-raw-query,formatted-sql-query  # not SQLAlchemy/SQL; shell path is sanitized via shlex.quote
    ...
    def test_glob_recursive_pattern(
        ...
        sandbox_backend.execute(  # nosemgrep: sqlalchemy-execute-raw-query,formatted-sql-query  # not SQLAlchemy/SQL; args are passed via shlex.join
    ...
    def test_glob_with_directories(
        ...
        sandbox_backend.execute(  # nosemgrep: sqlalchemy-execute-raw-query,formatted-sql-query  # not SQLAlchemy/SQL; args are passed via shlex.join
            shlex.join(["mkdir", "-p", f"{base_dir}/dir1", f"{base_dir}/dir2"])
        )
        sandbox_backend.write(f"{base_dir}/file.txt", "content")
    ...
    def test_glob_hidden_files_explicitly(
        ...
        mkdir_cmd = "mkdir -p " + _quote(base_dir)  # shell path is sanitized via shlex.quote
        sandbox_backend.execute(mkdir_cmd)  # nosemgrep: sqlalchemy-execute-raw-query  # not SQLAlchemy; shell path is sanitized via shlex.quote
