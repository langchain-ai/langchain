...
class FirecrawlScrapeWebsiteTool(BaseTool):
    ...
    description: str = (
        ...
        "Provide the URL of the website to scrape and indicate if the scraping rate should be limited. "
    ...
    api_key: str = Field(default_factory=lambda: os.getenv('FIRECRAWL_API_KEY', ''), description="API key for Firecrawl")
    limit_rate: bool = Field(default_factory=lambda: os.getenv('FIRECRAWL_LIMIT_RATE', 'True').strip().lower() == 'true', description="Limit scraping rate")

    def __init__(self, api_key: Optional[str] = None, limit_rate: Optional[bool] = None):
        super().__init__()
        self.api_key = api_key or self.api_key
        self.limit_rate = limit_rate if limit_rate is not None else self.limit_rate
        if not self.api_key:
            raise ValueError("API key for Firecrawl is required. Please set the FIRECRAWL_API_KEY environment variable or pass it to the constructor.")
    ...
    def map_website(self, app: FirecrawlApp, url: str) -> List[str]:
        try:
            ...
            if isinstance(map_status, dict) and 'links' in map_status:
                ...
                logging.warning("Map status is not a list or does not contain 'links': %s", map_status)
                return []
