...
class FinancePolygonReferenceTickerSchema(BaseModel):
    ...
    exchange: str = Field(
        description="specify primary exchange of asset in"
        "ISO code format",
    )
    cusip: str = Field(
    ...
    sort: str = Field(
      description="sort based on a field"
    )
...
class PolygonReferenceTicker(BaseTool):  # type: ignore[override, override]
    ...
    args_schema: Type[FinancePolygonReferenceTickerSchema] = FinancePolygonReferenceTickerSchema
