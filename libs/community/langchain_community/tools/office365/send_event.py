...
class O365SendEvent(O365BaseTool):
    ...
    def _run(
        ...
        event.start = dt.strptime(start_datetime, UTC_FORMAT).replace(tzinfo=ZoneInfo(timezone))
        event.end = dt.strptime(end_datetime, UTC_FORMAT).replace(tzinfo=ZoneInfo(timezone))
