...
class TerminalLiteralInputTool(BaseTool):
    ...
    def _run(self, literal_keys: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> None:
        TmuxPane(session_name=self.session_name, **PANE_KWARGS).send_keys(literal_keys, literal=True, enter=True)

class TerminalSpecialInputTool(BaseTool):
    """Tool to send a special key or key combination to a terminal window, followed by Enter"""
    ...
    def _run(self, special_key: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> None:
        TmuxPane(session_name=self.session_name, **PANE_KWARGS).send_keys(special_key, literal=False, enter=False)
...
class TerminalBottomCaptureTool(BaseTool):
    ...
    description: str = f"Use this tool to see the bottom N (up to {PANE_KWARGS['window_height']}) rows of the terminal window."
    session_name: str = None
    """Optional session name allows for different agents to have different terminal sessions if desired."""

    def _run(self, n_bottom_str: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> list[str]:
        ...
        n_bottom_rows = TmuxPane(session_name=self.session_name, **PANE_KWARGS).capture()[-n_bottom:]
...
class TerminalTopCaptureTool(BaseTool):
    ...
    description: str = (
        f"Use this tool to see the top N (up to {PANE_KWARGS['window_height']}) rows of the terminal window."
        "Note that terminal output always starts from the bottom, so you'll only see anything in the top row "
        f"when there have been {PANE_KWARGS['window_height']} rows of output."
    )
    session_name: str = None
    """Optional session name allows for different agents to have different terminal sessions if desired."""

    def _run(self, n_top_str: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> list[str]:
        # Ensure we can't exceed the window height
        n_top = min(int(n_top_str), PANE_KWARGS["window_height"])
        # Grab the pane contents
        pane_contents = TmuxPane(session_name=self.session_name, **PANE_KWARGS).capture()
        # Pad with leading empty strings if necessary
        pane_contents = [""] * (PANE_KWARGS["window_height"] - len(pane_contents)) + pane_contents
