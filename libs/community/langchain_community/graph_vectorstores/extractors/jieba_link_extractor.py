...
class JiebaLinkExtractor(LinkExtractor[KeybertInput]):
    def __init__(
        ...
        `jieba` is a popular Chinese text segmentation library and the `analyse` module can extract keywords from Chinese document.
        The JiebaLinkExtractor uses jieba.analyse to create links between documents that have keywords in common.
        Example::
            extractor = JiebaLinkExtractor()
            results = extractor.extract_one("小明硕士毕业于中国科学院计算所，后在日本京都大学深造")

            
        How to apply JiebaLinkExtractor to langchain_community.graph_vectorstores.extractors.LinkExtractorTransformer
        Example::
        ...
        Arguments:
            ...
            weight: Only works when the analyzer is `mixed`. Must be a tuple of length 2 and sum to 1.
        ...
        Example::
            extractor = JiebaLinkExtractor(analyzer='mixed', weight=(0.3, 0.7), extract_keywords_kwargs={'topK': 5, 'withWeight': True, 'allowPOS': ('n','nr','ns','nt','nz')})
            results = extractor.extract_one("小明硕士毕业于中国科学院计算所，后在日本京都大学深造")
        """
        try:
            ...
            if extract_keywords_kwargs is None:
                extract_keywords_kwargs = {"topK": 3, "withWeight": False, "allowPOS": ('n','nr','ns','nt','nz','a','an','v','vn'), "withFlag":False}
    ...
    def extract_one(
        ...
        if self._analyzer == 'tfidf':
            ...
            if len(self._weight) != 2 or self._weight[0]+self._weight[1] != 1:
                raise ValueError("when analyzer is 'mixed', weight must be a tuple of length 2 and sum to 1")
            keywords_tfidf:Dict[str,Tuple[str, float]] = {keyword[0]:keyword for keyword in TFIDF().extract_tags(input, **self._extract_keywords_kwargs)}
            keywords_textrank:Dict[str,Tuple[str, float]] = {keyword[0]:keyword for keyword in TextRank().extract_tags(input, **self._extract_keywords_kwargs)}
            mixed_keywords = {}
            for key, value in keywords_tfidf.items():
                if key in keywords_textrank:
                    mixed_keywords[key] = (key, value[1] * self._weight[0] + keywords_textrank[key][1] * self._weight[1])
                else:
