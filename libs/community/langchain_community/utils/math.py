...
def _cosine_sim_np(X: np.ndarray, Y: np.ndarray) -> np.ndarray:
...
def _cosine_sim_simd(X: np.ndarray, Y: np.ndarray) -> np.ndarray:
...
@functools.lru_cache
def _get_cdist_func() -> Callable:
    try:
        import simsimd as simd  # type: ignore

        return _cosine_sim_simd
    except ImportError:
        logger.info(
            "Unable to import simsimd, defaulting to NumPy implementation. If you want "
            "to use simsimd please install with `pip install simsimd`."
        )
        return _cosine_sim_np
...
def cosine_similarity(X: Matrix, Y: Matrix) -> np.ndarray:
    ...
    similarity_func = _get_cdist_func()
