...
class ChatTongyi(BaseChatModel):
    ...
    def fix_response_text(self, resp: Any) -> Any:
        """reponse ` {"role": "assistant", "content": [{"text": "图像"}]}}]}` is not working for langchain
        """
        ...
        if isinstance(message.get('content'), list):
            content_text = "".join(item.get('text', '') for item in message['content'] if isinstance(item, dict))
