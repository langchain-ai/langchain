...
# Remove deprecated import since this is a new module
...
def _create_retry_decorator(
    ...
    async def acompletion_with_retry(
        ...
        **kwargs: Any,
        ...
        api_specific_params = {
            "integrity", "integrity_model", "force_provider", "tools_model",
            "mem_session", "mem_expire", "mem_clear", "mem_msgs", "mem_length",
            "rag_tune", "routing"
        }
        openai_kwargs = {
...
class ChatAPIpie(BaseChatModel):
    ...
    top_p: float = 1.0
    """Nucleus sampling parameter, controls the cumulative probability distribution cutoff."""
    top_k: int = 0
    """Limits the selection pool to the top k most probable tokens."""
    ...
    when using one of the many model providers that expose an APIpie-like 
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["apipie_api_key"] = convert_to_secret_str(
            get_from_dict_or_env(
                values, "apipie_api_key", "APIPIE_API_KEY"
            )
        )
        ...
        # For testing purposes, if we're using a mock API key, don't validate against real API
        if values["apipie_api_key"].get_secret_value() in ["foo", "test_key"]:
            # For test_model_validation, we need to check against the mocked available_models
            if "available_models" in values and values["available_models"] is not None:
                ...
                if model_name not in values["available_models"] and model_id not in values["available_models"]:
                    raise ValueError(
        ...
        try:
            ...
            # Also check if the model is in provider/id format and extract just the id part
            model_id = model_name
            if "/" in model_name:
                _, model_id = model_name.split("/", 1)
