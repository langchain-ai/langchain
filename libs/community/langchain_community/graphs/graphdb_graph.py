...
class GraphDBGraph:
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def __init__(
        ...
        :param query_ontology: a `CONSTRUCT` query that is executed on the SPARQL endpoint and
        returns the KG schema statements
        Example: 'CONSTRUCT {?s ?p ?o} FROM <https://example.com/ontology/> WHERE {?s ?p ?o}'
        ...
        are not supported, because DESCRIBE returns the Symmetric Concise Bounded Description (SCBD),
        i.e. also the incoming class links. In case of large graphs with a million of instances, this is not efficient.
    ...
    def _get_auth() -> Union[tuple, None]:
        ...
        if username:
            if not password:
                raise ValueError(
                    'Environment variable \'GRAPHDB_USERNAME\' is set, but \'GRAPHDB_PASSWORD\' is not set.'
                )
            else:
    ...
    def _validate_user_query(query_ontology: str) -> None:
        ...
        if parsed_query.algebra.name != 'ConstructQuery':
            raise ValueError(f'Invalid query type. Only CONSTRUCT queries are supported.')
