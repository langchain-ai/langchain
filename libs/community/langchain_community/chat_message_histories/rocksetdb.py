...
class RocksetChatMessageHistory(BaseChatMessageHistory):
            ...
            lambda message_id: len(
                self._query(
                    f"""
                        ...
                        WHERE message.data.data.id = :message_id
                        LIMIT 1
                    """,
                    session_id=self.session_id,
    ...
    def __init__(
        ...
        Args:
            ...
            - message_uuid_method: The method that generates message IDs.
                    ...
                    `data` property. If this param is not set
    ...
    def add_message(self, message: BaseMessage) -> None:
        ...
        """
        if self.sync and "id" not in message.data:
            message.data["id"] = self.message_uuid_method()
        ...
        if self.sync:
            self._wait_until_message_added(message.data["id"])
