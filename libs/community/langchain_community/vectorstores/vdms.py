...
class IndexEngine(str, enum.Enum):
    """
    Enumerator of the underlying implementation for indexing and
    computing distances
    """
...
class VDMSVectorSearch(VectorStore):
    ...
    def __init__(
        ...
        distance_strategy: DistanceMetric = DEFAULT_DISTANCE_METRIC,
        engine: IndexEngine = DEFAULT_SEARCH_ENGINE,
    ...
    #     return self.collection_properties
    ...
    def check_required_inputs(self):
        ...
        # Check Eb=mbedding Func is provided and store dimension size
        if self._embedding_function is None:
            raise ValueError("Must provide embedding function")
        self._embedding_dimension = int(
            len(self._embedding_function.embed_query("text"))
        )
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        if ids is None:
            ids = [str(uuid.uuid1()) for _ in texts]
        # if hasattr(
        #     vdms_collection._client, "max_batch_size"
        # ):
        #     from vdmsdb.utils.batch_utils import create_batches

        #     for batch in create_batches(
        #         api=vdms_collection._client,
        #         ids=ids,
        #         metadatas=metadatas,
        #         documents=texts,
        #     ):
    ...
    def get(
        ...
        include: Optional[List[str]] = ["metadata"],
    ) -> Tuple[
        Any, Any
    ]:  # Tuple[List[Dict[str, Dict[str, Any]]], List[Union[bytes, None]]]:
        """Gets the collection.
    ...
    def add_collection_set(
        ...
        descriptor_set: Dict[str, Any] = {}
        descriptor_set["name"] = collection_name
        descriptor_set["dimensions"] = self._embedding_dimension
        ...
        response, response_array = self._client.query(all_queries)
    ...
    def get_entries(
        ...
        Args:
            ...
            limit: The number of documents to return. Optional.
            include: A list of what to include in the results.
                     Can contain `"embeddings"`, `"metadatas"`.
                     Ids are always included. Defaults to `["metadatas"]`.
                     Optional.
...
def _results_to_docs_and_scores(results: Any) -> List[Tuple[Document, float]]:
    ...
    # for qres in results:
    qres = results[0]
    if "FindDescriptor" in qres[0][0] and "entities" in qres[0][0]["FindDescriptor"]:
        result_entities = qres[0][0]["FindDescriptor"]["entities"]
        # result_blobs = qres[1]
