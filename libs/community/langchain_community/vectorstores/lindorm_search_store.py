...
from typing import Any, Dict, Iterable, List, Set, Optional, Tuple, Sequence
...
def _get_lindorm_search_client(lindorm_search_url: str, **kwargs: Any) -> Any:
    """Get lindorm search client through `opensearchpy` base on the lindorm_search_url, otherwise raise error."""
    try:
...
def embedding_thread(embedding: Embeddings,
    ...
    def embedding_and_write_queue(data_bulk):
        try:
            ...
            if expect_dimension != len(embeddings_bulk[0]):
                logger.error(
                    f"Embedding dimension dismatch, expect [{expect_dimension}], get [{len(embeddings_bulk[0])}], try set kwargs 'dimension' when initializing lindorm search store")
                raise RuntimeError("Embedding dimension dismatch!")
            embeddings_queue.put([texts_bulk, embeddings_bulk, ids_bulk, metadatas_bulk], timeout=1)
        except queue.Full:
            raise RuntimeError(f"Embedding queue is full with maxsize:{embeddings_queue.maxsize}")
    ...
    while not keyboard_stop_event.is_set():
        try:
            ...
            logger.error(f"RetryError in embedding:{e.last_attempt.exception()}")
            keyboard_stop_event.set()
    logger.info("embedding_thread stop")


def bulk_ingest_embedding_thread(embeddings_queue: queue.Queue,
...
class LindormSearchStore(VectorStore):
        ...
        try:
            ...
            if parent_index is not None:
                if parent_index == self.index_name:
                    raise RuntimeError(f"not allow index inherit the same index: {parent_index} == {self.index_name}")
            ...
            if parent_index is not None:
                ...
                if embedding is None:
                    raise RuntimeError(f"embedding server return None while embedding query: demo")
                routing_field = kwargs.get("routing_field", None)
                routing = "0"
                return_ids = _bulk_ingest_embeddings(
                    self.client,
    ...
    async def adelete_by_query(self, body: Any, params: Any = None, headers: Any = None, ) -> Any:
        return await run_in_executor(None, self.delete_by_query, body, params, headers)

    def delete_by_query(self, body: Any, params: Any = None, headers: Any = None, ) -> Any:
        def delete_route_by_query(route: str, body: Any, params: Any):
            ...
            return self.client.delete_by_query(index=self.index_name, body=body, params=params)

        if "routing" in params:
            return delete_route_by_query(params["routing"], body, params)
        print("body: {}".format(body))
        return self.client.delete_by_query(index=self.index_name, body=body, params=params, headers=headers)
    ...
    def start_working_threads(
        ...
        if self.embed_jobs is None and self.write_jobs is None:
            self.keyboard_stop_event.clear()

            self.embed_jobs = []
            for _ in range(embed_thread_num):
                embed_t = Thread(target=embedding_thread,
                                 args=(self.embedding,
                                       self.texts_queue,
            ...
            for _ in range(write_thread_num):
                write_t = Thread(target=bulk_ingest_embedding_thread,
                                 args=(self.embeddings_queue,
    ...
    def check_alive_working_thread(self):
        embed_thread_alive_num = 0
        for t in self.embed_jobs:
            if t.is_alive():
                embed_thread_alive_num += 1
        logger.info(f"embed thread: alive {embed_thread_alive_num}, total {len(self.embed_jobs)}")

        write_thread_alive_num = 0
        for t in self.write_jobs:
            if t.is_alive():
                write_thread_alive_num += 1
        logger.info(f"write thread: alive {write_thread_alive_num}, total {len(self.write_jobs)}")
    ...
    def __add_texts(
        ...
        self.progress_bar = tqdm(total=self.total_done + total_items,
                                 initial=self.total_done,
                                 )

        for offset in range(0, total_items, bulk_size):
            texts_bulk = texts[offset: offset + bulk_size]
            metadatas_bulk = metadatas[offset: offset + bulk_size] if metadatas else None
        ...
        while not self.keyboard_stop_event.is_set():
            try:
                ...
                # when call add_texts in a loop, and use KeyboardInterrupt to stop a iteration,
                # some residual data from last iteration may not read in time, so here use '>='
                ...
                # clean existing task. However, data in return_ids_queue is successfully written
                # into lsearch, it is not necessary to clear it.
                self.keyboard_stop_event.set()
                ...
                # close current progress bar to avoid being triggerred twice or more time in notebook.
                self.total_done = self.progress_bar.n
                self.progress_bar.close()
                raise KeyboardInterrupt("Received KeyboardInterrupt, interrupt current task...")
        self.refresh()
        return return_ids
    ...
    def add_texts(
        ...
        return self.__add_texts(texts=texts,
                                metadatas=metadatas,
                                ids=ids,
                                bulk_size=bulk_size)
    ...
    def _get_search_query(self, query: str, k: int = 4, **kwargs) -> Dict:
        ...
        if search_type == "approximate_search":
            ...
            if embedding is None:
                raise RuntimeError(f"embedding server return None while embedding query: {query}")
    ...
    def _multi_query_similarity_with_score(self, **kwargs: Any) -> Tuple[List[Document], float]:
        ...
        for single_kwargs in multi_kwargs:
            ...
            if "ext" in single_query_args and "lvector" in single_query_args["ext"]:
                single_query_args["ext"]["lvector"]["hybrid_search_type"] = "multi_filter_rrf"
    ...
    @staticmethod
    def _process_response(response: dict, text_field: str, metadata_field: str) -> List[Tuple[Document, float]]:
    ...
    def check_allow_inherit(self, parent_index: str, mapping: Dict):
        response = self.client.transport.perform_request(method="GET", url=f"/{parent_index}/_mapping?pretty")
        ...
        response = self.client.transport.perform_request(method="GET", url=f"/{parent_index}/_settings?pretty")
    ...
    def from_texts(
        ...
        Example:
            .. code-block:: python
                ...
                embeddings = LindormAIEmbeddings(endpoint='http://{instanceid}-proxy-ml.lindorm.rds.aliyuncs.com:9002',
                                                 username='root',
                                                 password='xxx',
        ...
        LindormSearch currently by default supports Approximate Search powered by lvector engines.
        ...
        Optional Keyword Args for Approximate Search:
            ...
            nlist: Number of cluster centersdefault. Between 2 and 1000000; default: 1000

            centroids_use_hnsw: Whether to use the HNSW algorithm when searching for cluster centers; default: False

            centroids_hnsw_m: Between 1 and 100; defualt: 16

            centroids_hnsw_ef_search: Size of the dynamic list used during k-NN searches. Higher values
            lead to more accurate but slower searches; default: 100

            centroids_hnsw_ef_construct: Size of the dynamic list used during k-NN graph creation.
        ...
        lindorm_search_store.add_texts(texts=texts,
                                       metadatas=metadatas,
                                       ids=ids,
                                       bulk_size=bulk_size)
        return lindorm_search_store
