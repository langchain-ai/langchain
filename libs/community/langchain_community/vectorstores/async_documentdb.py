...
class AsyncDocumentDBVectorSearch(VectorStore):
    ...
    - the `motor` python package installed
    ...
    Example:
        . code-block:: python
            ...
            mongo_client = await AsyncIOMotorClient("<YOUR-CONNECTION-STRING>")
            collection = await mongo_client["<db_name>"]["<collection_name>"]
    ...
    def __init__(
        self,
        collection: AgnosticCollection[DocumentDBDocumentType],
        embedding: Embeddings,
        *,
        index_name: str = "vectorSearchIndex",
        text_key: str = "textContent",
        embedding_key: str = "vectorContent",
        similarity_type: DocumentDBSimilarityType = DocumentDBSimilarityType.COS,
    ):
        ...
        Args:
            ...
            embedding_key: MongoDB field that will contain the embedding
                for each document.
            similarity_type: Method for comparing vector similarity
        """
    ...
    async def adelete_index(self) -> None:
        ...
        exists."""
        if self.aindex_exists():
            await self._collection.drop_index(self._index_name)
    ...
    async def aadd_texts(
        ...
        **kwargs: Any,
        ...
        for i, (text, metadata) in enumerate(zip(texts, _metadatas)):
            ...
            if (i + 1) % batch_size == 0:
                new_result_ids = await self._insert_texts(texts_batch, metadatas_batch)
    ...
    async def from_texts(
        ...
        collection: Optional[AgnosticCollection[DocumentDBDocumentType]] = None,
        **kwargs: Any,
    ) -> AsyncDocumentDBVectorSearch:
