...
class ClovaEmbeddings(BaseModel, Embeddings):
    ...
    To use this service, you should have the following environment variables set with your API tokens and application ID, or pass them as named parameters to the constructor:
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["clova_emb_apigw_api_key"] = convert_to_secret_str(
            get_from_dict_or_env(values, "clova_emb_apigw_api_key", "CLOVA_EMB_APIGW_API_KEY")
        )
    ...
    def _embed_text(self, text: str) -> List[float]:
        ...
        # send request
        response = requests.post(f"{self.endpoint_url}/{self.model}/{self.app_id.get_secret_value()}", headers=headers, json=payload)
