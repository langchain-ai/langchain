...
from langchain_core.pydantic_v1 import BaseModel, PrivateAttr, root_validator

def is_endpoint_live(url, headers, payload):
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            raise Exception(
                f"Endpoint returned a non-successful status code: {response.status_code}")
...
class NeMoEmbeddings(BaseModel, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        url = values.get("api_endpoint_url")
        model = values.get("model")

        print(f"Checking if endpoint is live: {url}")
        ...
        is_endpoint_live(url=url, headers=headers, payload=payload)

        return values

    async def _aembedding_func(self,
                               session,
                               text: str,
                               input_type: str) -> List[float]:
        ...
        async with session.post(self.api_endpoint_url,
                                json={"input": text,
                                      "model": self.model,
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for batch in range(0, len(texts), self.batch_size):
                ...
                for text in text_batch:
                    ...
                    tasks = [self._aembedding_func(
                        session, text, "passage") for text in text_batch]
