...
class OptimizedSentenceTransformer(sentence_transformers.SentenceTransformer):
    ...
    def _load_sbert_model(
        ...
        """
        # Check if the config_sentence_transformers.json file exists
        config_sentence_transformers_json_path = (
        ...
        for module_config in modules_config:
            module_class = sentence_transformers.util.import_from_string(
                module_config["type"]
            )
            # For Transformer, don't load the full directory,
            # rely on `transformers` instead. But, do load the config file first.
            if (
                module_class == sentence_transformers.models.Transformer
                and module_config["path"] == ""
            ):
                kwargs = {}
                for config_name in [
    ...
    def _load_sbert_model(
        ...
        for module_config in modules_config:
            ...
            if (
                ...
                module_class = InstructorEmbedding.import_from_string(
                    module_config["type"]
                )
                module_path = sentence_transformers.util.load_dir_path(
                    model_name_or_path,
