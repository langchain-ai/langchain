...
class OpenSkyAPIWrapper(BaseModel):
    ...
    def parse_flight_data(self, flight_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for flight in flight_data:
            parsed_flight = {
                ...
                "First Seen": datetime.fromtimestamp(flight.get("firstSeen")).isoformat(),
    ...
    def parse_state_vectors(self, timestamp: int, states: List[List[Any]]) -> List[Dict[str, Any]]:
        ...
        for state in states:
            parsed_state = {
                ...
                "Last Position Update": datetime.fromtimestamp(state[3]).isoformat() if state[3] else None,  # time_position
                "Last Contact": datetime.fromtimestamp(state[4]).isoformat() if state[4] else None,  # last_contact
                ...
                "Position Source": position_source_mapping.get(state[16], "Unknown")  # Get position source description
            }
