...
def get_loader_full_path(loader: BaseLoader) -> str:
    ...
    try:
        if "bucket" in loader_dict:
            ...
            if "channel" in loader_dict:
                location = location + "/" + loader_dict.get("channel", "-")
        elif "path" in loader_dict:
            location = loader_dict.get("path", "-")
        elif "file_path" in loader_dict:
            location = loader_dict.get("file_path", "-")
        elif "web_paths" in loader_dict:
            location = loader_dict.get("web_paths", ["-"])[0]
        # For in-memory types:
