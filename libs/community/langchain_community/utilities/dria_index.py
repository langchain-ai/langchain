...
class DriaAPIWrapper:
    ...
    This wrapper facilitates interactions with Dria's vector search and retrieval services,
    including creating knowledge bases, inserting data, and fetching search results.
    ...
    def query_with_vector(self, vector: List[float]):
    ...
    def run(self, query: Union[str, List[float]]):
        """Method to handle both text-based searches and vector-based queries.

        Args:
            query: A string for text-based search or a list of floats for vector-based query.
        ...
        if isinstance(query, str):
            ...
            logger.error(
                "Invalid query type. Please provide a string for text search or a list of floats for vector query."
            )
            return None
