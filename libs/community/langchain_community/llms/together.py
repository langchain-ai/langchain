...
class Together(LLM):
    ...
    def _initiate_request(
        ...
        **kwargs: Any,
        ...
        """Prepare and perform the initial request to the Together API."""
        headers = {
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        if self.streaming:
            ...
            response = self._initiate_request(prompt=prompt, stop=stop, **kwargs)
            data = response.json()
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        response = self._initiate_request(
            prompt=prompt,
            stop=stop,
            stream_tokens=True,
            **kwargs,
            ...
            for line in response.iter_lines():
                ...
                if line:
                    ...
                    chunk = GenerationChunk(
                        ...
                        generation_info={"logprob": token_data["token"]["logprob"]},
                    )
                    yield chunk
                    if run_manager:
                        run_manager.on_llm_new_token(
                            token_data["choices"][0]["text"],
                            verbose=self.verbose,
                        )
        except json.decoder.JSONDecodeError as e:
            raise Exception(
                f"""Error during streaming Didn't catch the end probably,
                and got this error: {e}"""
            )

        except requests.exceptions.RequestException as e:
            raise Exception("Error during streaming:", e)
