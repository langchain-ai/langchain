...
# this should trigger the guardrails - you can change this to any text you want which will trigger the guardrails
GUARDRAILS_TRIGGER = os.environ.get("GUARDRAILS_TRIGGERING_QUERY", "I want to talk about politics.")
...
def test_claude_instant_v1(bedrock_runtime_client, bedrock_models):
    try:
        llm = Bedrock(model_id='anthropic.claude-instant-v1', client=bedrock_runtime_client, model_kwargs={})
...
def test_amazon_bedrock_guardrails_no_intervention_for_valid_query(bedrock_runtime_client, bedrock_models):
    try:
        llm = Bedrock(model_id='anthropic.claude-instant-v1',
                      client=bedrock_runtime_client, model_kwargs={},
                      guardrails={"id": GUARDRAILS_ID, "version": GUARDRAILS_VERSION, "trace": False})
        output = llm("Say something positive:")
        assert isinstance(output, str)
    except Exception as e:
        pytest.fail(f"can not instantiate claude-instant-v1: {e}", pytrace=False)
...
def test_amazon_bedrock_guardrails_intervention_for_invalid_query(bedrock_runtime_client, bedrock_models):
    try:
        handler = BedrockAsyncCallbackHandler()
        llm = Bedrock(model_id='anthropic.claude-instant-v1',
                      client=bedrock_runtime_client, model_kwargs={},
                      guardrails={"id": GUARDRAILS_ID, "version": GUARDRAILS_VERSION, "trace": True},
                      callbacks=[handler])
        output = llm(GUARDRAILS_TRIGGER)
        guardrails_intervened = handler.get_response()
        assert guardrails_intervened == True
