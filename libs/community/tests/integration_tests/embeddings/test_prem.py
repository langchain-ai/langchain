"""Test PremEmbeddings from PremAI API wrapper.
...
@pytest.fixture
def embedder() -> PremEmbeddings:
    return PremEmbeddings(project_id=8, model="text-embedding-3-small")


def test_prem_embedding_documents(embedder: PremEmbeddings) -> None:
...
def test_prem_embedding_documents_multiple(embedder: PremEmbeddings) -> None:
...
def test_prem_embedding_query(embedder: PremEmbeddings) -> None:
