...
from langchain_community.chat_models.volcengine_maas import VolcEngineMaasChat, VolcEngineMaasChatV3
...
def test_default_call_v3() -> None:
    """Test valid chat call to volc engine."""
    chat = VolcEngineMaasChatV3()  # type: ignore[call-arg]
    response = chat.invoke([HumanMessage(content="Hello")])
    assert isinstance(response, BaseMessage)
    assert isinstance(response.content, str)
...
def test_multiple_history_v3() -> None:
    """Tests multiple history works."""
    chat = VolcEngineMaasChatV3()  # type: ignore[call-arg]
    response = chat.invoke(
        [
...
def test_stream_v3() -> None:
    """Test that stream works."""
    chat = VolcEngineMaasChatV3(streaming=True)  # type: ignore[call-arg]
    callback_handler = FakeCallbackHandler()
    callback_manager = CallbackManager([callback_handler])
    response = chat.invoke(
        [
...
def test_multiple_messages_v3() -> None:
    """Tests multiple messages works."""
    chat = VolcEngineMaasChatV3()  # type: ignore[call-arg]
    message = HumanMessage(content="Hi, how are you?")
    response = chat.generate([[message], [message]])
