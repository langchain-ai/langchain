...
def test_get_schema_from_file() -> None:
    graph_xml = GraphDBGraph(
        query_endpoint="http://localhost:7200/repositories/langchain",
        local_file=os.path.join(EXAMPLE_DIRECTORY, "starwars-ontology.xml"),
    )
    graph_n3 = GraphDBGraph(
        query_endpoint="http://localhost:7200/repositories/langchain",
        local_file=os.path.join(EXAMPLE_DIRECTORY, "starwars-ontology.n3"),
    )
    graph_ttl = GraphDBGraph(
        query_endpoint="http://localhost:7200/repositories/langchain",
        local_file=os.path.join(EXAMPLE_DIRECTORY, "starwars-ontology.ttl"),
    )
    graph_nt = GraphDBGraph(
        query_endpoint="http://localhost:7200/repositories/langchain",
        local_file=os.path.join(EXAMPLE_DIRECTORY, "starwars-ontology.nt"),
    )
    graph_trig = GraphDBGraph(
        query_endpoint="http://localhost:7200/repositories/langchain",
        local_file=os.path.join(EXAMPLE_DIRECTORY, "starwars-ontology.trig"),
    )
    graph_nq = GraphDBGraph(
        query_endpoint="http://localhost:7200/repositories/langchain",
        local_file=os.path.join(EXAMPLE_DIRECTORY, "starwars-ontology.nq"),
    )
    graph_jsonld = GraphDBGraph(
        query_endpoint="http://localhost:7200/repositories/langchain",
        local_file=os.path.join(EXAMPLE_DIRECTORY, "starwars-ontology.jsonld"),
    )
