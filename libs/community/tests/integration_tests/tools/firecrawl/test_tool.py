...
@pytest.mark.asyncio
async def test_integration_scrape_all_urls(mock_firecrawl_app):
...
@pytest.mark.asyncio
async def test_integration_map_website(mock_firecrawl_app):
    tool = FirecrawlScrapeWebsiteTool(api_key='fake_api_key')
    urls = tool.map_website(mock_firecrawl_app, 'https://example.com')
    assert urls == ['https://example.com/page1', 'https://example.com/page2']

@pytest.mark.asyncio
async def test_integration_async_scrape_url(mock_firecrawl_app):
    tool = FirecrawlScrapeWebsiteTool(api_key='fake_api_key')
    content = await tool.async_scrape_url(mock_firecrawl_app, 'https://example.com/page1')
    assert content == 'Sample markdown content'

@pytest.mark.asyncio
async def test_integration_error_handling_in_map_website():
    with patch('app.FirecrawlApp') as MockFirecrawlApp:
        mock_app = MockFirecrawlApp.return_value
...
@pytest.mark.asyncio
async def test_integration_error_handling_in_async_scrape_url():
    with patch('app.FirecrawlApp') as MockFirecrawlApp:
        mock_app = MockFirecrawlApp.return_value
