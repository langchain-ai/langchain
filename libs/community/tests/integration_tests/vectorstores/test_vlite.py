...
def test_vlite_with_metadatas() -> None:
    ...
    docsearch = VLite.from_texts(texts=texts, embedding=FakeEmbeddings(), metadatas=metadatas)
    output = docsearch.similarity_search("foo", k=1)
...
def test_vlite_update_document() -> None:
    ...
    texts = ["foo", "bar", "baz"]
    docsearch = VLite.from_texts(texts=texts, embedding=FakeEmbeddings(), ids=["1", "2", "3"])
...
def test_vlite_get_documents() -> None:
    ...
    texts = ["foo", "bar", "baz"]
    metadatas = [{"page": str(i)} for i in range(len(texts))]
    docsearch = VLite.from_texts(texts=texts, embedding=FakeEmbeddings(), metadatas=metadatas, ids=["1", "2", "3"])
...
def test_vlite_from_existing_index() -> None:
    ...
    texts = ["foo", "bar", "baz"]
    docsearch = VLite.from_texts(texts=texts, embedding=FakeEmbeddings(), collection="test_collection")
    new_docsearch = VLite.from_existing_index(collection="test_collection", embedding=FakeEmbeddings())
