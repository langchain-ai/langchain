...
def test_vlite_with_metadatas() -> None:
    ...
    docsearch = VLite.from_texts(
        texts=texts, embedding=FakeEmbeddings(), metadatas=metadatas  # type: ignore[call-arg]
    )
    output = docsearch.similarity_search("foo", k=1)
...
def test_vlite_update_document() -> None:
    ...
    texts = ["foo", "bar", "baz"]
    docsearch = VLite.from_texts(
        texts=texts, embedding=FakeEmbeddings(), ids=["1", "2", "3"]  # type: ignore[call-arg]
    )
...
def test_vlite_from_existing_index() -> None:
    ...
    texts = ["foo", "bar", "baz"]
    VLite.from_texts(
        texts=texts, embedding=FakeEmbeddings(), collection="test_collection"  # type: ignore[call-arg]
    )
    new_docsearch = VLite.from_existing_index(
        collection="test_collection", embedding=FakeEmbeddings()  # type: ignore[call-arg]
    )
