...
def test_APIFileLoader_calls_get_elements_from_api(
    ...
    assert docs[0].metadata.get("element_id") is not None
    assert set(fake_element_response[0].metadata.to_dict().keys()).issubset(
...
def test_APIFileLoader_with_multiple_files_calls_get_elements_from_api(
    ...
    ).issubset(set(docs[0].metadata.keys()))
        ...
        [
            {
                ...
                "text": "LayoutParser: A Uniﬁed Toolkit for Deep Learning Based"
