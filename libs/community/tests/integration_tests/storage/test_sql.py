...
def test_mget(sql_engine: Engine) -> None:
    ...
    with sql_engine.connect() as session:
        session.execute(text("insert into docstore ('namespace', 'key', 'value') "
                             "values('test','key1',:value)")
                        .bindparams(value=pickle.dumps(b"value1")),
                        )
        session.execute(text("insert into docstore ('namespace', 'key', 'value') "
                             "values('test','key2',:value)")
                        .bindparams(value=pickle.dumps(b"value2")),
                        )
        session.commit()
...
async def test_amget(sql_aengine: AsyncEngine) -> None:
    ...
    async with sql_aengine.connect() as session:
        await session.execute(text("insert into docstore ('namespace', 'key', 'value') "
                             "values('test','key1',:value)")
                        .bindparams(value=pickle.dumps(b"value1")),
                        )
        await session.execute(text("insert into docstore ('namespace', 'key', 'value') "
...
def test_mset(sql_engine: Engine) -> None:
    ...
    with sql_engine.connect() as session:
        result = session.exec_driver_sql("select * from docstore")
...
@pytest.mark.asyncio
async def test_amset(sql_aengine: Engine) -> None:
    """Test that multiple keys can be set."""
    store = SQLStore(engine=sql_aengine, namespace="test")
    await store.acreate_schema()
    key_value_pairs = [("key1", b"value1"), ("key2", b"value2")]
    ...
    async with sql_aengine.connect() as session:
        result = await session.exec_driver_sql("select * from docstore")
        assert result.keys() == ["namespace", "key", "value"]
        data = [(row[0], row[1]) for row in result]
        assert data == [("test", "key1"), ("test", "key2")]
        session.commit()
...
@pytest.mark.asyncio
async def test_amdelete(sql_aengine: Engine) -> None:
    """Test that deletion works as expected."""
    store = SQLStore(engine=sql_aengine, namespace="test")
    await store.acreate_schema()
    keys = ["key1", "key2"]
...
@pytest.mark.asyncio
async def test_yield_keys(sql_aengine: Engine) -> None:
    store = SQLStore(engine=sql_aengine, namespace="test")
    await store.acreate_schema()
    key_value_pairs = [("key1", b"value1"), ("key2", b"value2")]
    await store.amset(key_value_pairs)
    assert sorted([k async for k in store.ayield_keys()]) == ["key1", "key2"]
    assert sorted([k async for k in store.ayield_keys(prefix="key")]) == ["key1", "key2"]
