...
def test_imessage_chat_loader_upgrade_osx11() -> None:
    ...
    assert (
        first_message.output_metadata["message_time"] == expected_message_time
    ), "unexpected time"

    expected_parsed_time = datetime.datetime(2023, 11, 5, 2, 50, 50, 393148)
    assert (
        first_message.output_metadata["message_time_as_datetime"]
        == expected_parsed_time
    ), "date failed to parse"

    # is_from_me parsed correctly
    assert (
        first_message.output_metadata["is_from_me"] is False
