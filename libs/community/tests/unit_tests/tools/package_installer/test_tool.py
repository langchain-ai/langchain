...
def test_package_install_input_single():
    # Test single package input
    input_obj = PackageInstallInput(package_names="pandas")
    assert input_obj.package_names == "pandas", "Package names do not match for single package"


def test_package_install_input_multiple():
    # Test multiple package input
    input_obj = PackageInstallInput(package_names=["pandas", "numpy"])
    assert input_obj.package_names == ["pandas", "numpy"], "Package names do not match for multiple packages"
