...
class TestDirectoryLoader:
    ...
    def test_directory_loader_with_multithreading_enabled(self) -> None:
        ...
        loader = DirectoryLoader(dir_path, glob="**/*.csv", loader_cls=CSVLoader, use_multithreading=True)

        expected_docs = [
            Document(page_content="column1: value1", metadata={'source': self._get_csv_file_path("test_one_col.csv"), "row": 0}),
            Document(page_content="column1: value2", metadata={'source': self._get_csv_file_path("test_one_col.csv"), 'row': 1}),
            Document(page_content="column1: value3", metadata={'source': self._get_csv_file_path("test_one_col.csv"), 'row': 2}),
            Document(page_content="column1: value1\ncolumn2: value2\ncolumn3: value3", metadata={'source': self._get_csv_file_path("test_one_row.csv"), 'row': 0}),
            Document(page_content="column1: value1\ncolumn2: value2\ncolumn3: value3", metadata={'source': self._get_csv_file_path("test_nominal.csv"), 'row': 0}),
            Document(page_content="column1: value4\ncolumn2: value5\ncolumn3: value6", metadata={'source': self._get_csv_file_path("test_nominal.csv"), 'row': 1}),
        ]
