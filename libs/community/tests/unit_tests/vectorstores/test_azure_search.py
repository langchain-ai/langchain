...
def create_vector_store(additional_search_client_options: Optional[Dict[str, Any]] = None) -> AzureSearch:
    return AzureSearch(
...
def test_additional_search_options() -> None:
    ...
    with patch.multiple(
        ...
        vector_store = create_vector_store(additional_search_client_options={ "api_version": "test" })
        assert vector_store.client is not None
