...
from langchain_community.callbacks import MetricsCallbackHandler
from langchain_community.llms.openai import BaseOpenAI


@pytest.fixture
def handler() -> MetricsCallbackHandler:
    return MetricsCallbackHandler()


def test_on_llm_end(handler: MetricsCallbackHandler) -> None:
    response = LLMResult(
        generations=[],
        llm_output={
            "token_usage": {
...
def test_on_llm_end_custom_model(handler: MetricsCallbackHandler) -> None:
    response = LLMResult(
        generations=[],
        llm_output={
            "token_usage": {
...
def test_on_llm_end_finetuned_model(
    handler: MetricsCallbackHandler, model_name: str, expected_cost: float
) -> None:
    response = LLMResult(
        generations=[],
        llm_output={
...
def test_on_llm_end_no_cost_invalid_model(
    handler: MetricsCallbackHandler, model_name: str
) -> None:
    response = LLMResult(
        generations=[],
        llm_output={
...
def test_on_retry_works(handler: MetricsCallbackHandler) -> None:
