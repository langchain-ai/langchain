...
from langchain_community.callbacks import AsyncOpenAICallbackHandler
from langchain_community.llms.openai import BaseOpenAI


@pytest.fixture
def handler() -> AsyncOpenAICallbackHandler:
    return AsyncOpenAICallbackHandler()


@pytest.mark.asyncio
async def test_on_llm_end(handler: AsyncOpenAICallbackHandler) -> None:
    response = LLMResult(
        generations=[],
        llm_output={
            "token_usage": {
...
@pytest.mark.asyncio
async def test_on_llm_end_with_chat_generation(
    handler: AsyncOpenAICallbackHandler,
) -> None:
    response = LLMResult(
        generations=[
            [
...
@pytest.mark.asyncio
async def test_on_llm_end_with_chat_generation_no_cost(
    handler: AsyncOpenAICallbackHandler,
) -> None:
    response = LLMResult(
        generations=[],
...
@pytest.mark.asyncio
async def test_on_llm_end_finetuned_model(
    handler: AsyncOpenAICallbackHandler, model_name: str, expected_cost: float
) -> None:
    response = LLMResult(
        generations=[],
        llm_output={
...
@pytest.mark.asyncio
async def test_on_llm_end_azure_openai(
    handler: AsyncOpenAICallbackHandler, model_name: str, expected_cost: float
) -> None:
    response = LLMResult(
        generations=[],
...
@pytest.mark.asyncio
async def test_on_llm_end_no_cost_invalid_model(
    handler: AsyncOpenAICallbackHandler, model_name: str
) -> None:
    response = LLMResult(
        generations=[],
        llm_output={
