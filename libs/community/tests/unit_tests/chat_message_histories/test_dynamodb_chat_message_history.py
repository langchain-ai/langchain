...
class MockDynamoDBChatHistoryTable:
    ...
    class Table:
        ...
        def update_item(
            ...
            update_dict = {"History": ExpressionAttributeValues[":h"]}
...
class TestDynamoDBChatMessageHistory:
    def test_add_message(
        ...
        assert item_after_human_message["History"] == messages_to_dict(
            [first_message]
        )  # History should only contain the first message
        assert (
            item_after_human_message["TimeToLive"]
        ...
        assert item_after_ai_message["History"] == messages_to_dict(
            [first_message, second_message]
        )  # Second message should have appended
        assert (
            item_after_ai_message["TimeToLive"]
            == mock_time_2 + chat_history_config["ttl"]
