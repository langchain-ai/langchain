...
class OllamaLLM(BaseLLM):
    ...
    """The number of GPUs to use. Defaults to ``1`` on macOS (to enable metal
    support). Set to ``0`` to disable."""
    ...
    ``100``) will give more diverse answers, while a lower value (e.g. ``10``)
    ...
    Use ``sync_client_kwargs`` and ``async_client_kwargs`` to pass different
    arguments to synchronous and asynchronous clients.
    """
