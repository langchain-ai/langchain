...
`__version__` variable in `langchain_requesty/_version.py`. Intended for use as a
...
def main() -> int:
    ...
    version_path = package_dir / "langchain_requesty" / "_version.py"
    ...
    if version_py_version is None:
        print("Error: Could not find __version__ in langchain_requesty/_version.py")  # noqa: T201
        return 1
    ...
    if pyproject_version != version_py_version:
        ...
        print(f"  langchain_requesty/_version.py: {version_py_version}")  # noqa: T201
        return 1
