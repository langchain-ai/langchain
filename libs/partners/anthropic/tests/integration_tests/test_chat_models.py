...
async def test_agent_loop_streaming_astream_events_v3_v1() -> None:
    ...
    llm_with_tools = llm.bind_tools([get_weather])
    input_message = HumanMessage("What is the weather in San Francisco, CA?")
    tool_call_message = await (await llm_with_tools.astream_events([input_message], version="v3"))
    assert isinstance(tool_call_message, AIMessage)
    tool_calls = tool_call_message.tool_calls
    assert len(tool_calls) == 1
    tool_call = tool_calls[0]
    ...
    assert isinstance(tool_message, ToolMessage)
    response = await (
        await llm_with_tools.astream_events(
            [input_message, tool_call_message, tool_message],
            version="v3",
        )
    )
...
def test_streaming_tool_call_v1_v2_parity() -> None:
    """`AIMessage` parity between `stream()` reduction and `stream_events(version="v3").output`.
