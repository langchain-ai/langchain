...
def _create_usage_metadata(anthropic_usage: BaseModel) -> UsageMetadata:
    ...
    Note:
        Anthropic's ``input_tokens`` represents only the non-cached portion of
        input tokens (i.e. tokens billed at the standard input rate).  Tokens
        served from the prompt cache are reported separately as
        ``cache_read_input_tokens``, and tokens written to the cache are
        reported as ``cache_creation_input_tokens``.  These values are
        surfaced in ``UsageMetadata.input_token_details`` and are **not**
        included in ``UsageMetadata.input_tokens``, which mirrors the raw
        Anthropic API field.  ``total_tokens`` accounts for all token types:
        non-cached input, cache reads, cache creation, and output.
    """
    cache_read = getattr(anthropic_usage, "cache_read_input_tokens", None)
    cache_creation_flat = getattr(anthropic_usage, "cache_creation_input_tokens", None)

    input_token_details: dict = {
        "cache_read": cache_read,
        "cache_creation": cache_creation_flat,
    }

    # Add cache TTL information if provided (5-minute and 1-hour ephemeral cache)
    cache_creation_obj = getattr(anthropic_usage, "cache_creation", None)

    # Currently just copying over the 5m and 1h keys, but if more are added in
    # the future we'll need to expand this tuple.
    ...
    if cache_creation_obj:
        if isinstance(cache_creation_obj, BaseModel):
            cache_creation_obj = cache_creation_obj.model_dump()
        for k in cache_creation_keys:
            specific_cache_creation_tokens += cache_creation_obj.get(k, 0)
            input_token_details[k] = cache_creation_obj.get(k)
    ...
    # `input_tokens` mirrors Anthropic's raw field: non-cached tokens only.
    # Cache tokens are tracked separately in `input_token_details`.
    input_tokens = getattr(anthropic_usage, "input_tokens", 0) or 0
    output_tokens = getattr(anthropic_usage, "output_tokens", 0) or 0

    # `total_tokens` includes every token type so callers can compute overall
    # usage without having to inspect `input_token_details`.
    total_tokens = (
        input_tokens
        + (cache_read or 0)
        + (specific_cache_creation_tokens or cache_creation_flat or 0)
        + output_tokens
    )
