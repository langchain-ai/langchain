...
def test_ark_embeddings_reduce() -> None:
    from langchain_ark.utils import sliced_norm_l2, cosine_similarity
    embeddings = ArkEmbeddings()
    ...
    assert cosine_similarity(vector_reduced, vector_a_reduced) > cosine_similarity(vector_reduced, vector_b_reduced)
