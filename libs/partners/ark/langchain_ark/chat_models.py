...
class ChatArk(BaseChatModel):
    """`Ark` Chat large language models API.

    To use, you should have the
    environment variable `ARK_API_KEY` set with your API key.

    Any parameters that are valid to be passed to the ark.create call
    can be passed in, even if not explicitly saved on this class.

    Setup:
        Install `langchain-ark` and set environment variable
        ```ARK_API_KEY```.

        .. code-block:: bash

            pip install -U langchain-ark
            export ARK_API_KEY="your-api-key"

    Key init args — completion params:
        model: str
            Name of ark model to use. "model_endpoint_id".
        temperature: float
    ...
    Key init args — client params:
        ...
        api_key: Optional[str]
            ARK API key. If not passed in will be read from env var ARK_API_KEY.
    ...
    ark_api_key: Optional[SecretStr] = Field(
        alias="api_key", default_factory=secret_from_env("ARK_API_KEY", default=None)
    )
    """Automatically inferred from env var `ARK_API_KEY` if not provided."""
    ark_api_base: Optional[str] = Field(
        alias="base_url",
        default_factory=from_env(["ARK_API_BASE", "ARK_BASE_URL"], default="https://ark.cn-beijing.volces.com/api/v3"),
    )
    """Base URL path for API requests, leave blank if not using a proxy or service
        emulator."""
    # to support explicit proxy for ARK
    ark_proxy: Optional[str] = Field(
        default_factory=from_env("ARK_PROXY", default=None)
    )
    request_timeout: Union[float, Tuple[float, float], Any, None] = Field(
        default=None, alias="timeout"
    )
    """Timeout for requests to ARK completion API. Can be float, httpx.Timeout or
        None."""
    max_retries: int = 2
    ...
    def validate_environment(self) -> Self:
        ...
        client_params: Dict[str, Any] = {
            "api_key": (
                self.ark_api_key.get_secret_value() if self.ark_api_key else None
            ),
        ...
        try:
            ...
            raise ImportError(
                "Could not import ark python package. "
    ...
    def _default_params(self) -> Dict[str, Any]:
        """Get the default parameters for calling ARK API."""
        params = {
    ...
    @deprecated(
        since="0.2.0",
        alternative="langchain_ark.chat_models.ChatARK.bind_tools",
        removal="0.3.0",
    )
    def bind_functions(
            self,
        ...
        Args:
            ...
            **kwargs: Any additional parameters to pass to
                :meth:`~langchain_ark.chat_models.ChatARK.bind`.
        """
    ...
    def with_structured_output(
        ...
        Example: schema=Pydantic class, method="function_calling", include_raw=False:
            .. code-block:: python
                ...
                llm = ChatArk(model="model-endpoint-id", temperature=0)
...
def _convert_chunk_to_message_chunk(
    ...
    if role == "user" or default_class == HumanMessageChunk:
        ...
        if usage := (chunk.get("x_ark") or {}).get("usage"):
...
def _lc_tool_call_to_ark_tool_call(tool_call: ToolCall) -> dict:
    return {
        "type": "function",
        "id": tool_call["id"],
        "function": {
