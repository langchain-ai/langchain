...
def test_it_gets_content_from_file_path(mock_file: Mock):
    loader = _UnstructuredBaseLoader(file_path="dummy_path")
...
def test_it_raises_value_error_without_file_or_file_path():
    loader = _UnstructuredBaseLoader()
...
# --- _UnstructuredBaseLoader._elements_json ---


def test_it_calls_elements_via_api_with_valid_args():
    with patch.object(
        _UnstructuredBaseLoader, '_elements_via_api', new_callable=mock.PropertyMock
...
@patch.object(_UnstructuredBaseLoader, '_convert_elements_to_dicts')
def test_it_partitions_locally_by_default(mock_convert_elements_to_dicts: Mock):
    mock_convert_elements_to_dicts.return_value = [{}]
    with patch.object(
        _UnstructuredBaseLoader, '_elements_via_local', new_callable=mock.PropertyMock
...
def test_it_partitions_locally_and_logs_warning_with_partition_via_api_False(
    ...
    with patch.object(_UnstructuredBaseLoader, '_elements_via_local') as mock_get_elements_locally:
        mock_get_elements_locally.return_value = [Text("Mock text element.")]
        loader = _UnstructuredBaseLoader(partition_via_api=False, api_key="some_key")
...
def test_it_raises_value_error_with_partition_via_api_True_and_no_api_key():
    loader = _UnstructuredBaseLoader(partition_via_api=True, api_key=None)
