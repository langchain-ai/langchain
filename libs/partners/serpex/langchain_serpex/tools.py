...
class SerpexSearchResults(BaseTool):
    ...
    engine: str = Field(default="auto", description="Search engine: auto, google, bing, duckduckgo, brave, yahoo, yandex")
    category: str = Field(default="web", description="Search category (currently only 'web' supported)")
    time_range: Optional[str] = Field(default=None, description="Time range: all, day, week, month, year (not supported by Brave)")
    ...
    def validate_environment(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        """Validate that API key exists in environment or is provided."""
        api_key = values.get("api_key")
        
        if not api_key or (isinstance(api_key, SecretStr) and not api_key.get_secret_value()):
            api_key_str = os.getenv("SERPEX_API_KEY")
            if not api_key_str:
                raise ValueError(
                    "SERPEX API key is required. "
                    "Set it as SERPEX_API_KEY environment variable "
                    "or pass it as 'api_key' parameter."
                )
            values["api_key"] = SecretStr(api_key_str)
        elif isinstance(api_key, str):
            values["api_key"] = SecretStr(api_key)
        
        # Also check for custom base URL
        if "base_url" not in values or not values["base_url"]:
            base_url = os.getenv("SERPEX_BASE_URL")
            if base_url:
    ...
    def _build_params(self, query: str, **kwargs: Any) -> Dict[str, Any]:
        """Build the query parameters for the SERPEX API request."""
        params: Dict[str, Any] = {
            "q": query,
    ...
    def _format_results(self, data: Dict[str, Any]) -> str:
        ...
        if "infoboxes" in data and isinstance(data["infoboxes"], list) and len(data["infoboxes"]) > 0:
        ...
        if "results" in data and isinstance(data["results"], list) and len(data["results"]) > 0:
            num_results = data.get("metadata", {}).get("number_of_results", len(data["results"]))
        ...
        # Search suggestions
        if not results_parts and "suggestions" in data and isinstance(data["suggestions"], list) and len(data["suggestions"]) > 0:
            results_parts.append("No direct results found. Related searches:")
            results_parts.extend(data["suggestions"])
        
        # Query corrections
        if not results_parts and "corrections" in data and isinstance(data["corrections"], list) and len(data["corrections"]) > 0:
