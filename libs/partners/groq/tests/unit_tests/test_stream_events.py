"""Unit tests for the groq native stream-events converter."""
...
from langchain_groq._stream_events import aconvert_groq_stream, convert_groq_stream
...
def test_convert_groq_stream_lifecycle() -> None:
    events: list[Any] = list(convert_groq_stream(iter(_reasoning_text_tool())))
...
def test_groq_stream_events_v3_lifecycle() -> None:
    ...
    surfaces reasoning/text/tool blocks with `model_provider == "groq"`.
    """
    ...
    assert message_start["metadata"]["provider"] == "groq"
...
async def test_groq_astream_events_v3_lifecycle() -> None:
    """Async twin of `test_groq_stream_events_v3_lifecycle`."""
    llm = ChatGroq(model="foo")
...
async def test_aconvert_groq_stream_lifecycle() -> None:
    async def _araw() -> Any:
        for chunk in _reasoning_text_tool():
            yield chunk

    events: list[Any] = [e async for e in aconvert_groq_stream(_araw())]
    assert_valid_event_stream(events)
    assert events[0]["event"] == "message-start"
    assert events[0]["id"] == ""
    assert events[0]["metadata"]["provider"] == "groq"
...
def test_convert_groq_stream_parallel_tool_calls() -> None:
    """Args at distinct `tool:{idx}` keys finalize as separate tool calls."""
    events: list[Any] = list(convert_groq_stream(iter(_parallel_tool_calls())))
    assert_valid_event_stream(events)
