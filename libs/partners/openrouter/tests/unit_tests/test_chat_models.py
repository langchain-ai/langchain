...
class TestWrapMessagesForSdk:
    ...
    def test_wraps_as_pydantic_models(self) -> None:
        ...
        result = _wrap_messages_for_sdk(msgs)
        assert len(result) == 2
        assert isinstance(result[0], components.SystemMessage)  # type: ignore[arg-type]
        assert isinstance(result[1], components.UserMessage)  # type: ignore[arg-type]
    ...
    def test_all_roles_wrapped(self) -> None:
        ...
        assert isinstance(result[0], components.SystemMessage)  # type: ignore[arg-type]
        assert isinstance(result[1], components.UserMessage)  # type: ignore[arg-type]
        assert isinstance(result[2], components.AssistantMessage)  # type: ignore[arg-type]
        assert isinstance(result[3], components.ToolResponseMessage)  # type: ignore[arg-type]
