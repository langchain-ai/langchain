...
def _json_body_matcher(r1: Any, r2: Any) -> None:
    ...
    if isinstance(b2, bytes):
        b2 = b2.decode("utf-8")
    try:
        j1 = _normalize_function_call_status(
            _normalize_tool_descriptions(json.loads(b1))
        )
        j2 = _normalize_function_call_status(
            _normalize_tool_descriptions(json.loads(b2))
        )
