...
def test_bind_tools_with_provider_strategy_format() -> None:
    """Test that bind_tools accepts response_format in ProviderStrategy format.

    Reproduces issue #34797 where ProviderStrategy structure causes ValueError.
    """
    model = ChatOpenAI(model="gpt-4o", api_key="test")  # type: ignore[arg-type]
    schema: dict[str, Any] = {
        "type": "object",
        "required": ["result"],
        "properties": {
    ...
    # Simulate ProviderStrategy output
    response_format = {
        "type": "json_schema",
        "json_schema": {
            "name": "test_schema",
    ...
    # Should not raise
    model.bind_tools([], response_format=response_format)
    ...
    model.bind_tools([], response_format=response_format_strict)

    # Test with strict=False
    response_format_strict_false = {
        "type": "json_schema",
        "json_schema": {
            "name": "test_schema",
            "schema": schema,
    ...
    model.bind_tools([], response_format=response_format_strict_false)
