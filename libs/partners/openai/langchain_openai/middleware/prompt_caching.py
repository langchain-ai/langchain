"""OpenAI prompt caching middleware.

Requires:
    - langchain: For agent middleware framework
    - langchain-openai: For ChatOpenAI model (already a dependency)
"""
...
try:
    ...
    msg = (
        "OpenAIPromptCachingMiddleware requires 'langchain' to be installed. "
...
class OpenAIPromptCachingMiddleware(AgentMiddleware):
    ...
    Optimizes API usage by caching conversation prefixes for OpenAI models.

    Requires both 'langchain' and 'langchain-openai' packages to be installed.

    Learn more about OpenAI prompt caching
    ...
    def _should_apply_caching(self, request: ModelRequest) -> bool:
        ...
        if not isinstance(request.model, ChatOpenAI):
            msg = (
                ...
                f"OpenAI models, not instances of {type(request.model)}"
            )
