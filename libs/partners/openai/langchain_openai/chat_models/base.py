...
def _convert_dict_to_message(_dict: Mapping[str, Any]) -> BaseMessage:
    ...
    if role == "user":
        ...
        if function_call := _dict.get("function_call"):
            data["function_call"] = dict(function_call)
        if tool_calls := _dict.get("tool_calls"):
            data["tool_calls"] = tool_calls
        return AIMessage(content=content, data=data, id=id_)
    elif role == "system":
...
def _convert_message_to_dict(message: BaseMessage) -> dict:
    ...
    if isinstance(message, ChatMessage):
        ...
        if "tool_calls" in message.data:
            message_dict["tool_calls"] = message.data["tool_calls"]
...
def _convert_delta_to_message_chunk(
    ...
    if role == "user" or default_class == HumanMessageChunk:
        ...
        return AIMessageChunk(content=content, data=data, id=id_)
