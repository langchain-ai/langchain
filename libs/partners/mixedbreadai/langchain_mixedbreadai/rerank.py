...
class MixedbreadAIRerank(MixedBreadAIClient, BaseDocumentCompressor):
    ...
    This class utilizes the Mixedbread AI Rerank API to reorder documents based on their relevance
    to a given query. It supports both synchronous and asynchronous operations.

    Attributes:
        model (str): Model to use for reranking. Defaults to "mixedbread-ai/mxbai-rerank-large-v1".
    ...
    def rerank(
        ...
        Returns:
            Sequence[Dict]: A sequence of dictionaries containing document index and score.
        """
        if len(documents) == 0:
            return []
    ...
    async def arerank(
        ...
        results = (await self._aclient.reranking(
            model=self.model,
            query=query,
            input=docs,
            rank_fields=rank_fields,
    ...
    def compress_documents(
        ...
        Returns:
            Sequence[Document]: A sequence of compressed documents in relevance_score order.
        """
        if len(documents) == 0:
            return []
