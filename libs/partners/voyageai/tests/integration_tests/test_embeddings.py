...
def test_langchain_voyageai_embedding_documents() -> None:
    ...
    documents = ["foo bar"]
    embedding = VoyageAIEmbeddings(model=MODEL)  # type: ignore[call-arg, call-arg, call-arg]
...
def test_langchain_voyageai_embedding_documents_multiple() -> None:
    ...
    embedding = VoyageAIEmbeddings(model=MODEL, batch_size=2)  # type: ignore[call-arg, call-arg]
    output = embedding.embed_documents(documents)
    assert len(output) == 3
    assert len(output[0]) == 1024
