...
class BaseModeration:
    ...
    def _convert_text_to_prompt(self, prompt: Any, text: str) -> Any:
        if isinstance(prompt, StringPromptValue):
            ...
            if isinstance(message, HumanMessage):
                messages[self.chat_message_index] = HumanMessage(content=text, 
                                                                 example=message.example, 
                                                                 additional_kwargs=message.additional_kwargs)
            if isinstance(message, AIMessage):
                messages[self.chat_message_index] = AIMessage(content=text, 
                                                              example=message.example, 
                                                              additional_kwargs=message.additional_kwargs) 
    ...
    def _moderation_class(self, moderation_class):
        return moderation_class(client=self.client, 
                                callback = self.moderation_callback,
                                unique_id=self.unique_id,
                                chain_id = self.chain_id).validate
        ...
        try:
            ...
            if self.config is None:
                ...
                self._log_message_for_verbose(
                    f"Running pii validation...\n"
                    )
                pii_validate = self._moderation_class(moderation_class=ComprehendPII)
                output_text = pii_validate(prompt_value=input_text) 

                self._log_message_for_verbose(
                    f"Running toxicity validation...\n"
                    )
                toxicity_validate = self._moderation_class(moderation_class=ComprehendToxicity)
                output_text  = toxicity_validate(prompt_value=output_text)

                self._log_message_for_verbose(
                    f"Running intent validation...\n"
                    )
                intent_validate = self._moderation_class(moderation_class=ComprehendIntent)
                output_text  = intent_validate(prompt_value=output_text)
            else:
                filter_functions = { "pii": ComprehendPII,
                ...
                for _filter in filters:
                    ...
                    if filter_name in filter_functions:
                        ...
                        validation_fn = self._moderation_class(moderation_class=filter_functions[filter_name])                    
                        input_text = input_text if not output_text else output_text
                        output_text = validation_fn(prompt_value=input_text, 
                                                    config=self.config[filter_name] if filter_name in self.config else None)
            # convert text to prompt and return
            return self._convert_text_to_prompt(prompt=prompt,
                                                text=output_text)
