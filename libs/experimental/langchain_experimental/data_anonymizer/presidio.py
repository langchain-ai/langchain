...
class PresidioReversibleAnonymizer(ReversibleAnonymizerBase):
    ...
    def __init__(
        ...
        self._analyzer = AnalyzerEngine()
        self._anonymizer = AnonymizerEngine()
        self.deanonymizer_mapping: Dict[str, Dict[str, str]] = dict()
    ...
    def _create_update_deanonymizer_mapping(
        ...
        """Create a mapping from anonymized text to original text"""
        ...
        new_deanonymizer_mapping: Dict[str, Dict[str, str]] = {}
        ...
        for analyzed_entity, anonymized_entity in zip(
            ...
            if anonymized_entity.entity_type not in new_deanonymizer_mapping:
        ...
        for entity_type, values in new_deanonymizer_mapping.items():
            if entity_type not in self.deanonymizer_mapping:
                self.deanonymizer_mapping[entity_type] = {}

            self.deanonymizer_mapping[entity_type].update(values)
    ...
    def _deanonymize(self, text_to_deanonymize: str) -> str:
        for entity_type in self.deanonymizer_mapping:
            for anonymized, original in self.deanonymizer_mapping[entity_type].items():
