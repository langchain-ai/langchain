...
def _init_chat_model_helper(
    ...
    **kwargs: Any,
    ...
    if model_provider == "huggingface":
        ...
        for key in ["backend", "device", "device_map", "model_kwargs", "pipeline_kwargs", "batch_size"]:
            if key in kwargs:
                pipeline_specific_kwargs[key] = kwargs.pop(key)
        
        # Remaining kwargs (temperature, max_tokens, etc.) should go to pipeline_kwargs
        # for generation parameters, which ChatHuggingFace will inherit from the LLM
        ...
        for k, v in list(kwargs.items()):
            if k == "max_tokens":
                ...
                kwargs.pop(k)
            elif k in ("temperature", "max_new_tokens", "top_p", "top_k", "repetition_penalty", "do_sample"):
