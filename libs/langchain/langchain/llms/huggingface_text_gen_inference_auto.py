...
llm = HuggingFaceTextGenInferenceAuto.from_docker(model_name, host='0.0.0.0', port=8080, shm_size="1g")
...
try:
    ...
    # We want to make this module entirely optional, so allow module to be imported regardless
    # of whether user has python docker library installed. We'll raise an error at runtime if
    # they try to actually use any of the methods in the AutoHuggingFaceTextGenInference class.
    docker = None
    Container = None


SERVICE_NAME = 'text-generation-inference'   # This will be prefixed to container names, e.g. <service-name>--...
...
def check_os() -> Tuple[bool, str]:
    ...
    if os_type == 'darwin':
        if platform.machine() == 'arm64':
            return False, f"{__name__} module is not compatible with macOS with Apple Silicon."
        else:
            return True, f"{__name__} module hasn't been explicitly tested on macOS without Apple Silicon. YMMV."
    elif os_type == 'windows':
        return True, f"{__name__} hasn't been tested on Windows. YMMV."
    elif os_type == 'linux':
        if not is_debian_based():
            return True, f"{__name__} module hasn't been explicitly tested with non-Debian based Linux distros. YMMV."
        else:
            return True, ""
    else:
...
def check_nvidia_docker() -> bool:
    ...
    """
    try:
        result = subprocess.run(['docker', 'info', '--format', '{{.Runtimes.nvidia}}'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        return 'nvidia' in result.stdout
    except FileNotFoundError:
        return False
...
def check_prerequisites() -> Tuple[bool, str]:
    ...
    Returns a tuple containing a boolean and a message indicating whether the prerequisites are met.
    """
    if docker is None:
        docker_msg = (
...
# Define a simplified stat_result namedtuple with just a few fields,
# so that we mock the API of os.stat() method for consistency.
stat_result = namedtuple('stat_result', ['st_size'])
...
class DockerPath(pathlib.Path):
    ...
    def __new__(cls, container_id, *pathsegments):
    ...
    def stat(self):
        ...
        Returns:
            dict: A dictionary with the file size in bytes, or None if the path does not exist
        ...
        try:
            ...
            return None  # Output was not a valid integer, indicating an error
...
def hf_model_name_to_model_dir(name: str) -> str:
    ...
    n = name.split("/")
    if len(n) != 2:
        raise ValueError(f"Model name '{name}' is not a valid HF model name. "
...
def stop_container(client, name: str, timeout=30) -> None:
    ...
    if container is not None:
        ...
        if not container:
            ...
            raise ValueError("Docker container did not shut down properly (or didn't shut down before timeout)")
    else:
...
def find_available_host_port(
    ...
    if port_range[0] < 1024 or port_range[1] > 65535:
        raise ValueError(f'Port range must be in range 1024-65535, inclusive. Got: {port_range}')
    ...
    while not is_host_port_available(client, port):
        if retries >= max_retries or port >= port_range[1]:
            raise ValueError(f'No available port found within {max_retries} retries or port range {port_range}.')
        port += 1
        retries += 1
...
class DockerParams(_SharedParams):
    ...
    def container_name(self) -> str:
        """ Return a deterministic name for the container based on its parameter values. """
    ...
    def _validate(cls, values):
        ...
        if not os.path.isabs(values['volume_path']):
            raise ValueError(f"Volume path must be absolute. Got: {values['volume_path']}")
        values['volume_path'] = str(values['volume_path'])

        if not is_valid_hf_model_name_format(values['pretrained_model_name']):
            raise ValueError(f"Model '{values['pretrained_model_name']}' is not a valid HuggingFace model name"
    ...
    def compute_md5(self) -> str:
        ...
        params_str = json.dumps(params_dict, sort_keys=True)  # Serialize to JSON with sorted keys for consistency
        md5_hash = hashlib.md5(params_str.encode()).hexdigest()
        return md5_hash

    def equals_with_exclusions(self, other: 'DockerParams', exclude: Union[str, List[str]] = None) -> bool:
        """ Compare parameters, excluding those specified by an individual key or list of keys. """
...
class HuggingFaceTextGenInferenceAuto:
    ...
    DEFAULT_MODELS_CACHE_DIR = (Path('~').expanduser() / ".langchain-textgen" / "models").absolute()
    ...
    @staticmethod
    def _validate_cached_model(model_path_on_host: Path, model_path_on_container: DockerPath):

        # This is an area for improvement if we wanted to do some more validation, e.g. make sure the
        # model isn't corrupted, etc., or if we wanted to allow options for the user to force re-downloading
        # For now we'll just make this a dummy method and leave implementation to future work.

        # **** Area that can be improved in the future: add actual validation checks ****
        is_good_model = True
        if not is_good_model:
            raise ValueError(f"Cached model in {model_path_on_host} failed validation check.")
    ...
    def _wait_for_model_download(cls, container: Container) -> None:
        ...
        if container is None:
            raise ValueError(f"Existing container for model not found.")

        model_name = get_model_name_from_container(container)

        model_dir_name = hf_model_name_to_model_dir(model_name)
        cached_model_path_on_host, cached_model_path_on_container = cls._get_model_volume_path(container)
        cached_model_path_on_host = Path(cached_model_path_on_host) / model_dir_name
        cached_model_path_on_container = DockerPath(container.short_id, cached_model_path_on_container) / model_dir_name

        # Check if the model is already cached and that it matches on host and container.
        # Assumption is that we're bind mounting models into our
        # standard location in a hidden directory in the users home directory, e.g. .cache.,
        # and that it is mounted into the container at /data
        if str(os.path.join(*cached_model_path_on_container.parts[:2])) != "/data":
            raise ValueError(f"Models *must* be mounted in container to /data. Got: {cached_model_path_on_container}")

        if cached_model_path_on_host.exists() and cached_model_path_on_container.exists():
            cls._validate_cached_model(cached_model_path_on_host, cached_model_path_on_container)
            ...
            return
        elif not cached_model_path_on_host.exists() and cached_model_path_on_container.exists():
            # This is one of the edge cases I've found, where the user manually deletes a model in the
            # host cache while the model container is still running
            msg = (
                "Model volume cache mismatch. Model exists in container but doesn't exist on host. "
            )
        ...
        with CountdownProgressBar(cls.DOWNLOAD_TIMEOUT, desc='Downloading model [0mb complete] (timeout countdown)') as pbar:
            ...
            while not download_complete and time.time() - start_time < cls.DOWNLOAD_TIMEOUT:
                ...
                if download_match and success_match:
                    ...
                    model_size = bytes2human(get_directory_size(cached_model_path_on_host))
                    print(f"\nModel {model_name} download completed in {download_time} ({model_size})")
                    download_complete = True

                model_size = bytes2human(get_directory_size(cached_model_path_on_host))
                time.sleep(5)    # Wait for 5 seconds before checking again
                pbar.update(5, new_desc=f'Downloading model [{model_size} complete] (timeout countdown)')
    ...
    def _wait_for_status(cls, container, inference_server_url: str):
        ...
        is_api_ready = False
        with CountdownProgressBar(total=cls.HEALTH_CHECK_TIMEOUT, desc="Waiting for container/inference server to start") as pbar:
            start_time = time.time()
    ...
    def _create_reuse_or_recreate_container(
        ...
        """
        print("\n********** HuggingFaceTextGenInference Container/Server Startup Procedure **********")

        # This is the desired state the user wants
        target_params = docker_params

        # First list all running containers for our service, and get the params that were used to construct them
        containers_in_service = [c for c in client.containers.list() if c.name.startswith(SERVICE_NAME)]
        for existing_container in containers_in_service:
            existing_params = DockerParams.from_container(existing_container)

            # If we find a container that matches what we want already, we immediately return it
            if target_params.container_name == existing_params.container_name:
                print(f"Founding matching container that already exists. Re-using it.")
                return existing_container, docker_params, generation_params

            # If everything matches *except* the model the user wants to use, then we assume
            # the user wants to re-create the container using the new model
            if target_params.equals_with_exclusions(existing_params, exclude='pretrained_model_name'):
                ...
                print(
                    ...
                    f"Shutting down existing container and will restart with desired model."
                )
        ...
        # First check if any of the specified params are going to run into conflicts (e.g. host port availability)
        # If the port isn't available and AUTO_INCREMENT_PORT=True, try to find the next available port
        ...
        if not is_host_port_available(client, docker_params.port):
            ...
            if not cls.AUTO_INCREMENT_PORT:
                raise ValueError(
                    f"Host port {docker_params.port} unavailable and AUTO_INCREMENT_PORT={cls.AUTO_INCREMENT_PORT}"
                    f"Set AutoHuggingFaceTextGenInference.AUTO_INCREMENT_PORT=True to try to find available port"
                )
    ...
    def _from_params(
        ...
        container, docker_params, generation_params = cls._create_reuse_or_recreate_container(
            client,
            docker_params,
            generation_params
        )
        ...
        # Store the docker params in the additional server_kwargs key that Langchain provides.
    ...
    def get_docker_config_from_llm(cls, llm: HuggingFaceTextGenInference) -> dict:
        """ Get a dict of the docker params used to instantiate the textgen-inference-server.
    ...
    def from_docker(cls, model_name: str, **kwargs) -> HuggingFaceTextGenInference:
        ...
        kwargs['volume_path'] = Path(kwargs.pop('volume_path', cls.DEFAULT_MODELS_CACHE_DIR))
