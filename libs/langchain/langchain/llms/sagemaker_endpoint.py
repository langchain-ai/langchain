...
class SagemakerEndpoint(LLM):
            ...
            try:
                ...
                for line in iterator:
                    if is_lmi:
                        ...
                        resp_output = data["token"]["text"] if not data["token"]["special"] else ""
                    else:
                        continue
                    if stop is not None:
    ...
    async def _acall(
            ...
            **kwargs: Any,
            ...
            """
            Asynchronously calls the SageMaker inference endpoint with the given prompt.

            Args:
                prompt (str): The prompt to pass into the model.
                stop (Optional[List[str]]): Optional list of stop words to use when generating.
                run_manager (Optional[AsyncCallbackManagerForLLMRun]): An optional callback manager for tracking the generation progress.
                **kwargs: Additional keyword arguments to pass to the model.

            Returns:
                str: The string generated by the model.

            Raises:
                ValueError: If there is an error raised by the SageMaker endpoint.
