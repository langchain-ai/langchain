...
class EmailFromPDF(BasePDFLoader):
    ...
    def __init__(self,
        ...
        assert os.path.exists(tesseract_location+'.exe'), \
            ...
            tesseract_location+". Please install and/or specify the correct location."
        self.tesseract_location=tesseract_location
        super().__init__(file_path)
        self.headers=headers #was getting wiped out by --init--
    ...
    def parse_email(self,thedoc):
        ...
        first_head_pos=min((email_text.find(sub) for sub 
            in self.allheads if email_text.find(sub) != -1), default=None)
            ...
            for header in self.headers[key]:
                ...
                if match:
                    ...
                    if key in self.lists:
                        ...
                        elif fields[key].count(',')<=1: #if only one ","" and no ";"s
        ...
        if self.date_tag in fields: #change date to unix timestamp at start of day
            try:    #don't want to die on malformed date
                ...
                date = date.replace(hour=0, minute=0, second=0, 
                                    microsecond=0) #go back to midnight
