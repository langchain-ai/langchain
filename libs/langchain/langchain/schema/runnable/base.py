...
class RunnableBinding(RunnableSerializable[Input, Output]):
    ...
    def _merge_configs(self, *configs: Optional[RunnableConfig]) -> RunnableConfig:
        """Merge configs; merging is order sensitive.

        The precedence is:

        1. The config passed to this runnable at run time
        2. The config factories passed to this runnable at instantiation
        3. The configs passed to this runnable at instantiation

        Args:
            *configs: Run time configs to merge (these have the highest precedence).
