...
class BaseMemoryAsync(Serializable, ABC):
    ...
    Example:
        .. code-block:: python
            ...
            class SimpleMemory(BaseMemoryAsync):
                memories: Dict[str, Any] = dict()

                @property
                async def memory_variables(self) -> List[str]:
                    return list(self.memories.keys())
...
class BaseChatMessageHistoryAsync(ABC):
    ...
    See `ChatMessageHistory` for the default implementation.

    Example:
        .. code-block:: python

            class FileChatMessageHistory(BaseChatMessageHistoryAsync):
                storage_path: str
                session_id: str

                @property
                async def messages(self):
                    with open(os.path.join(self.storage_path, self.session_id), 'r:utf-8') as f:
                        messages = json.loads(f.read())
                    return messages_from_dict(messages)
