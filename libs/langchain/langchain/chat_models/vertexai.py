...
def _history_to_str(
    ...
    prefix_ai: str = "Output",
    prefix_human: str = "Input",
) -> str:
    """Returns a chat history as a string"""
    history = []
    for message in message_history:
        if isinstance(message, AIMessage):
            history.append(f"{prefix_ai}: {message.content}")
        elif isinstance(message, HumanMessage):
            history.append(f"{prefix_human}: {message.content}")
    return "\n".join(history)
...
class TextChatVertexAI(_VertexAICommon, BaseChatModel):
    """A chat model based on text version of Vertex LLM."""

    model_name: str = "text-bison"
    default_prompt: str = (
        "Predict the next message in the chat given the chat "
        "history:\n\n{history}\nOutput:"
    )
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if isinstance(messages[0], SystemMessage):
            prompt = messages[0].content.format(history=_history_to_str(messages[:-1]))
        else:
            prompt = self.default_prompt.format(history=_history_to_str(messages[:-1]))
        params = {**self._default_params, **kwargs}
        text = self._predict(prompt, stop, **params)
        return ChatResult(generations=[ChatGeneration(message=AIMessage(content=text))])
