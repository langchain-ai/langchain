...
class BaseRetrievalStrategy(ABC):
    ...
    def index(
        ...
        vector_query_field: str,
        similarityAlgo: Union[DistanceStrategy, None],
        ...
        return {
            "settings": {
                "index": {
                    ...
                    "bpack.knn.hnsw.m": index_params.get("m", 16),
                    "bpack.knn.hnsw.ef_construction": index_params.get("ef_construction", 512),
                }
            },
            "mappings": {
                "properties": {
...
class BESVerctorStore(VectorStore):
    ...
    Args:
        ...
        strategy: Optional. Retrieval strategy to use when searching the index.
                 Defaults to LinearRetrievalStrategy. Can be one of
                 LinearRetrievalStrategy or HNSWRetrievalStrategy
    ...
    def __init__(
        ...
        *,
        ...
        bes_client: Optional["Elasticsearch"] = None,
        bes_url: Optional[str] = None,
        bes_cloud_id: Optional[str] = None,
        ...
        if bes_client is not None:
            self.client = bes_client
        elif bes_url is not None or bes_cloud_id is not None:
            self.client = BESVerctorStore.connect_to_bes(
                bes_url=bes_url,
                username=user,
                password=password,
                cloud_id=bes_cloud_id,
                api_key=api_key,
            )
        else:
            raise ValueError(
    ...
    def _create_index_if_not_exists(
        ...
        if self.client.indices.exists(index=index_name):
            ...
            if self.distance_strategy is DistanceStrategy.COSINE:
                similarityAlgo = "cosine"
            elif self.distance_strategy is DistanceStrategy.EUCLIDEAN_DISTANCE:
                similarityAlgo = "l2_norm"
            elif self.distance_strategy is DistanceStrategy.DOT_PRODUCT:
    ...
    def _search(
        ...
        logger.debug(f"Query body: {query_body}, size:{k}")
    ...
    def _create_cls_from_kwargs(
        ...
        bes_client = kwargs.get("bes_client")
        cloud_id = kwargs.get("cloud_id")
        url = kwargs.get("bes_url")
