...
class Dingo(VectorStore):
    ...
    def add_texts(
        ...
        ids = ids or [int(str(uuid.uuid1().int)[:13]) for _ in texts]
        metadatas_list = []
        embeds = []
        ...
        for i in range(0, len(texts), batch_size):
            j = i + batch_size
            self._client.vector_add(self._index_name, metadatas_list[i:j], embeds[i:j], ids[i:j])
...
sss
        ...
        for i in range(0, len(texts), batch_size):
            j = i + batch_size
            dingo_client.vector_add(index_name, metadatas_list[i:j], embeds[i:j], ids[i:j])
