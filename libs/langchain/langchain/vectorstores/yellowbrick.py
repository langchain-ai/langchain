...
class Yellowbrick(VectorStore):
        ...
        **kwargs: Any,
        ...
        for id in range(len(embeddings)):
            ...
            insert_query = sql.SQL('INSERT INTO {t} \
                (id, embedding_id, text, metadata, embedding) VALUES {v}').format(
                    t=sql.Identifier(self._table),v=(sql.SQL(',').join([sql.SQL('(%s,%s,%s,%s,%s)') 
    ...
    def create_connection(connection_string: str) -> psycopg2.connection:
        """Create a connection to Yellowbrick
