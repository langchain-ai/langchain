...
class DGraphQAChain(Chain):
    ...
    def _call(
        ...
        while dql_result is None and generation_attempt <= self.max_generation_attempts:
        ...
        if dql_result is None:
            message = f"""Failed to generate a valid DQL query after 
                        {generation_attempt} attempts. Unable to execute 
                        DQL query because of the following error: {dql_error}"""
