...
from langchain.chains.graph_qa.prompts import (
    SPARQL_GRAPHDB_GENERATION_SELECT_PROMPT,
    SPARQL_GRAPHDB_REGENERATION_SELECT_PROMPT,
...
class GraphDBQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    sparql_regeneration_select_chain: LLMChain
    qa_chain: LLMChain
    ...
    def from_llm(
            ...
            *,
            qa_prompt: BasePromptTemplate = SPARQL_GRAPHDB_QA_PROMPT,
            sparql_select_prompt: BasePromptTemplate = SPARQL_GRAPHDB_GENERATION_SELECT_PROMPT,
            sparql_regeneration_select_prompt: BasePromptTemplate = SPARQL_GRAPHDB_REGENERATION_SELECT_PROMPT,
        ...
        sparql_regeneration_select_chain = LLMChain(llm=llm, prompt=sparql_regeneration_select_prompt)
        max_regeneration_attempts = max_regeneration_attempts
        return cls(
            qa_chain=qa_chain,
            sparql_generation_select_chain=sparql_generation_select_chain,
            sparql_regeneration_select_chain=sparql_regeneration_select_chain,
            max_regeneration_attempts=max_regeneration_attempts,
            **kwargs,
        )
    ...
    def _call(
        ...
        generated_sparql = self._get_valid_sparql_query(_run_manager, callbacks, generated_sparql)
        query_results = self._execute_sparql_query(_run_manager, generated_sparql)

        result = self.qa_chain({'prompt': prompt, 'context': query_results}, callbacks=callbacks)
    ...
    def _get_valid_sparql_query(
        ...
        try:
            ...
            while retries < self.max_regeneration_attempts:
                try:
                    generated_sparql = self.sparql_regeneration_select_chain.run(
                        {"parse_exception": parse_exception, "generated_sparql": generated_sparql},
                        callbacks=callbacks
                    )
                    return self._parse_sparql_query(_run_manager, generated_sparql)
                except ParseException as e:
