                ...
                if action == BaseModerationActions.STOP:
                    for item in response["ResultList"]:
                        for label in item["Labels"]:
                            if label and (not toxicity_labels or label["Name"] in toxicity_labels) and label["Score"] >= threshold:  
                                toxicity_found = True
                                break

                if action == BaseModerationActions.ALLOW: 
                    if not toxicity_labels:
                        warnings.warn("You have allowed toxic content without specifying any toxicity labels.")
                    else:
                        for item in response["ResultList"]:
                            for label in item["Labels"]:
