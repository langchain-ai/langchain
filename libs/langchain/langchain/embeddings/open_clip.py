...
class OpenCLIPEmbeddings(BaseModel, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            model, _, preprocess = open_clip.create_model_and_transforms(model_name=model_name, pretrained=checkpoint)
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        text_features = [self.model.encode_text(self.tokenizer(text)).tolist() for text in texts]
    ...
    def embed_image(self, images: List[float]) -> List[List[float]]:
        ...
        image_features = [self.model.encode_image(self.preprocess(pil_image).unsqueeze(0)).tolist() for pil_image in pil_images]
