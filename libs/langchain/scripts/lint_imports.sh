...
# make sure not importing from langchain_community
git --no-pager grep '^from langchain_community\.' `git ls-files langchain | grep -vE 'langchain/(agents/load_tools|chains/llm_requests|memory/chat_memory).py'` && errors=$((errors+1))
