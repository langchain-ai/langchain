...
@pytest.fixture
def cache_embedder() -> CacheBackedEmbeddings:
...
def test_embed_documents(cache_embedder: CacheBackedEmbeddings) -> None:
    texts = ["1", "22", "a", "333"]
    ...
    keys = list(cache_embedder.document_embedding_store.yield_keys())
    assert len(keys) == 4
    # UUID is expected to be the same for the same text
    assert keys[0] == "test_namespace812b86c1-8ebf-5483-95c6-c95cf2b52d12"


def test_embed_query(cache_embedder: CacheBackedEmbeddings) -> None:
    text = "query_text"
