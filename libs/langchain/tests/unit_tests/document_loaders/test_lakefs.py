...
def mock_lakefs_client() -> Any:
    with patch("langchain.document_loaders.lakefs.LakeFSClient") as mock_lakefs_client:
        mock_lakefs_client.return_value.ls_objects.return_value = [('path_bla.txt', 'https://physical_address_bla')]
...
def mock_unstructured_local() -> Any:
    with patch("langchain.document_loaders.lakefs.UnstructuredLakeFSLoader") as mock_unstructured_lakefs:
        mock_unstructured_lakefs.return_value.load.return_value = [('text content', 'pdf content')]
...
def mock_lakefs_client_no_presign_local() -> Any:
    with patch("langchain.document_loaders.lakefs.LakeFSClient") as mock_lakefs_client:
        mock_lakefs_client.return_value.ls_objects.return_value = [('path_bla.txt', 'local:///physical_address_bla')]
        mock_lakefs_client.return_value.is_presign_supported.return_value = False
        yield mock_lakefs_client.return_value
...
class TestLakeFSLoader(unittest.TestCase):
    ...
    def test_presigned_loading(self, mocker) -> None:
        mocker.register_uri('GET', requests_mock.ANY, text='data')
        loader = LakeFSLoader(self.lakefs_access_key,
                              self.lakefs_secret_key,
                              self.endpoint)
    ...
    def test_non_presigned_loading_fail(self, mocker) -> None:
    ...
    @requests_mock.Mocker()
    @pytest.mark.usefixtures("mock_lakefs_client_no_presign_local", "mock_unstructured_local")
    def test_non_presigned_loading(self, mocker) -> None:
        mocker.register_uri(requests_mock.ANY, requests_mock.ANY, status_code=200)
