...
def test_pandas_output_parser_row_no_array():
    try:
        parser.parse("row:5")
        assert False, "Should have raised OutputParserException"
    except OutputParserException:
...
def test_pandas_output_parser_row_first():
...
def test_pandas_output_parser_row_no_column():
    try:
...
def test_pandas_output_parser_row_col_1():
    expected_output = {'1': 2}
    actual_output = parser.parse("row:1[chicken]")
    assert actual_output == expected_output

def test_pandas_output_parser_special_ops():
    actual_output = [
        {'mean': 3.0},
        {'median': 3.0},
        {'min': 2},
...
def test_pandas_output_parser_invalid_special_op():
    try:
