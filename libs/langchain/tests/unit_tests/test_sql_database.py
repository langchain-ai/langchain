...
@pytest.mark.requires("aiosqlite", "nest_asyncio")
@pytest.mark.asyncio
async def test_table_info_async() -> None:
    """Test that table info is constructed properly."""
    engine = create_async_engine("sqlite+aiosqlite:///:memory:")
    async with engine.connect() as conn:
        await conn.run_sync(metadata_obj.create_all)
    db = SQLDatabase(engine)
    output = await db.table_info_async
    expected_output = """
    CREATE TABLE user (
...
@pytest.mark.requires("aiosqlite", "nest_asyncio")
@pytest.mark.asyncio
async def test_sql_database_run_async() -> None:
