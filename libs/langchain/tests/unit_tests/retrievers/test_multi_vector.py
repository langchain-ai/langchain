...
def test_multi_vector_retriever_similarity_search_with_score() -> None:
    ...
    retriever = MultiVectorRetriever(  # type: ignore[call-arg]
        vectorstore=vectorstore, docstore=InMemoryStore(), doc_id="doc_id", search_kwargs={"score_threshold": 0.5},
    ...
    retriever = MultiVectorRetriever(  # type: ignore[call-arg]
        vectorstore=vectorstore, docstore=InMemoryStore(), doc_id="doc_id", search_kwargs={"score_threshold": 0.9},
        search_type="similarity_score_threshold"
    )
    retriever.docstore.mset(list(zip(["1"], documents)))
    results = retriever.invoke("1")
