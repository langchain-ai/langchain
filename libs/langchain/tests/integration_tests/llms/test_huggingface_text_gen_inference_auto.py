...
def check_host_port_availability():
    ...
    for port in needed_ports:
        if not check_port_available(host, port):
            import warnings
            message = (
                f"Skipping tests. Port {port} is in use on the host. Ensure it is free then run tests again. "
            )
...
@pytest.mark.requires('docker')
@pytest.fixture(scope='session', autouse=True)
def check_compatibility_before_tests():
    """
...
def tolerant_tempdir():
    ...
    user_accessible_temp_dir = Path('~').expanduser() / '.tmp-langchain-integration-tests'
    ...
    try:
        ...
        try:
            ...
            # Enforce hardcoded path to make it harder to accidentally delete important stuff
            ...
            if str(user_accessible_temp_dir) == str(path2):
                ...
                raise ValueError("Temp path is hardcoded for safety. "
                                 "Modify path and assertion if you really mean to change it. ")
        except PermissionError:
            print(f"Encountered PermissionsError while closing temp_dir. Will need to manually clean up {temp_dir}.")
...
def docker_container():
    import docker
    client = docker.from_env()
    container = client.containers.run('alpine', command='/bin/sh', detach=True, tty=True)
    yield container
    container.stop()
    container.wait(timeout=30)
...
def setup_docker_containers_for_port_tests(docker_client):
    ...
    try:
        ...
        container1 = docker_client.containers.run(image_name, 'sleep infinity', detach=True, ports={'80/tcp': 8080})
        container2 = docker_client.containers.run(image_name, 'sleep infinity', detach=True, ports={'80/tcp': 8081})
        yield
    finally:
...
@pytest.mark.requires('docker')
@pytest.mark.slow
def test_find_available_port_no_auto_increment(docker_client, setup_docker_containers_for_port_tests):
...
@pytest.mark.requires('docker')
@pytest.mark.slow
def test_find_available_port_auto_increment(docker_client, setup_docker_containers_for_port_tests):
...
@pytest.mark.requires('docker')
@pytest.mark.slow
def test_find_available_port_max_retries(docker_client, setup_docker_containers_for_port_tests):
    with pytest.raises(ValueError) as exc_info:
        find_available_host_port(docker_client, 8080, auto_increment=True, max_retries=1)
    assert str(exc_info.value) == 'No available port found within 1 retries or port range (1024, 65535).'
...
def test_invalid_port_ranges(docker_client):
    ...
    assert str(exc_info.value).startswith('Port range must be in range 1024-65535, inclusive.')
...
def test_startup_shutdown(docker_client, temp_models_dir, model_name):
    ...
    llm = HuggingFaceTextGenInferenceAuto.from_docker(model_name, host='0.0.0.0', port=8081, shm_size="1g")
...
def test_container_model_name_swap(docker_client, temp_models_dir, model_name):
    ...
    llm_new = HuggingFaceTextGenInferenceAuto.from_docker(new_model_name, host='0.0.0.0', port=8081, shm_size="1g")
    d_new = DockerParams(**HuggingFaceTextGenInferenceAuto.get_docker_config_from_llm(llm_new))
    ...
    # Doing the model swap should have shut down existing container, and recreated with new model
    # running on the same port as the previous. Any other changes would have generated an auto-increment
    # to the port, so check that same port is still in use as a basic sanity test
