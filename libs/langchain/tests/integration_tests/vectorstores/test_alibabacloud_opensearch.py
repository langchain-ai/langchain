...
class FakeEmbeddingsWithOsDimension():
...
settings = AlibabaCloudOpenSearchSettings(
    endpoint="The endpoint of opensearch instance, If you want to access through
     ...
     information of the instance details. If you want to access within the Alibaba Cloud VPC,
     you can directly use the API domain name."
    ...
    namespace (str) : The instance data will be partitioned based on the "namespace" field.
     ...
     Otherwise, the queries cannot be executed correctly, default is empty.
    table_name="The table name is specified when adding a table after completing the instance configuration.",
    field_name_mapping={
...
def test_alibabacloud_opensearch_with_text_query() -> None:
    opensearch = create_alibabacloud_opensearch()
    output = opensearch.similarity_search(text="foo", k=1)
    assert output == [Document(page_content="foo",
                               metadata={'string_field': "value1", "int_field": 1, 'float_field': 1.0,
                                         'double_field': 2.0})]

    output = opensearch.similarity_search(text="bar", k=1)
    assert output == [Document(page_content="bar",
                               metadata={'string_field': "value2", "int_field": 2, 'float_field': 3.0,
                                         'double_field': 4.0})]

    output = opensearch.similarity_search(text="baz", k=1)
    assert output == [Document(page_content="baz",
                               metadata={'string_field': "value3", "int_field": 3, 'float_field': 5.0,
                                         'double_field': 6.0})]
...
def test_alibabacloud_opensearch_with_text_and_meta_query() -> None:
    ...
    output = opensearch.similarity_search(
        text="foo", search_filter={"string_field": "value1"}, k=1
    )
    assert output == [
        Document(page_content="foo", metadata={'string_field': "value1", 'int_field': 1, 'float_field': 1.0,
                                               'double_field': 2.0})]

    output = opensearch.similarity_search(
        text="bar", search_filter={'int_field': 2}, k=1
    )
    assert output == [
        Document(page_content="bar", metadata={'string_field': "value2", 'int_field': 2, 'float_field': 3.0,
                                               'double_field': 4.0})]

    output = opensearch.similarity_search(
        text="baz", search_filter={"float_field": 5.0}, k=1
    )
    assert output == [
        Document(page_content="baz", metadata={'string_field': "value3", "int_field": 3, 'float_field': 5.0,
                                               'double_field': 6.0})]

    output = opensearch.similarity_search(
        text="baz", search_filter={"float_field": 6.0}, k=1
    )
    assert len(output) == 0
...
def test_alibabacloud_opensearch_with_text_and_meta_score_query() -> None:
    ...
    output = opensearch.similarity_search_with_relevance_scores(
        text="foo", search_filter={'string_field': "value1", 'int_field': 1, 'float_field': 1.0, 'double_field': 2.0},
        k=1
    )
    assert output == [
