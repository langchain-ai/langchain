...
def test_valid_sparql(max_regeneration_attempts: int) -> None:
    ...
    chain.sparql_fix_select_chain = Mock(LLMChain)
    chain.qa_chain = Mock(LLMChain)

    chain.sparql_generation_select_chain.run = MagicMock(
        return_value="SELECT * {?s ?p ?o} LIMIT 1"
    )
    chain.sparql_fix_select_chain.run = MagicMock()
    ...
    assert chain.sparql_fix_select_chain.run.call_count == 0
    assert chain.qa_chain.call_count == 1
    assert result == "Tatooine"


@pytest.mark.parametrize("max_regeneration_attempts", [-2, -1, 0])
def test_invalid_sparql_non_positive_max_regeneration_attempts(
...
def test_valid_sparql_after_first_retry(max_regeneration_attempts: int) -> None:
    ...
    assert chain.sparql_generation_select_chain.run.call_count == 1
    assert chain.sparql_fix_select_chain.run.call_count == 1
    assert chain.qa_chain.call_count == 1
    assert result == "Tatooine"
...
def test_invalid_sparql_after_all_retries(max_regeneration_attempts: int) -> None:
    ...
    assert chain.sparql_generation_select_chain.run.call_count == 1
    assert chain.sparql_fix_select_chain.run.call_count == max_regeneration_attempts
    assert chain.qa_chain.call_count == 0


@pytest.mark.parametrize(
    "max_regeneration_attempts,number_of_invalid_responses",
    [(1, 0), (2, 0), (2, 1), (10, 6)],
)
def test_valid_sparql_after_some_retries(
    ...
    chain.sparql_generation_select_chain.run = MagicMock(
        return_value=invalid_sparql_query
    )
    chain.sparql_fix_select_chain.run = Mock()
    chain.sparql_fix_select_chain.run.side_effect = [
        invalid_sparql_query
    ] * number_of_invalid_responses + [valid_sparql_query]
    ...
    assert chain.sparql_generation_select_chain.run.call_count == 1
    assert (
        chain.sparql_fix_select_chain.run.call_count == number_of_invalid_responses + 1
    )
    assert chain.qa_chain.call_count == 1
    assert result == "Tatooine"
...
def query_examples(model_name: str, graph: GraphDBGraph) -> None:
    chain = GraphDBQAChain.from_llm(
        ChatOpenAI(temperature=0, model_name=model_name), graph=graph
    )
    try:
