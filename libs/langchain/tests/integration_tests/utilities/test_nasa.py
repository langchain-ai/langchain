...
def test_get_media_metadata_manifest() -> None:
    ...
    nasa = NasaAPIWrapper()
    output = nasa.run("get_media_metadata_manifest", '{"nasa_id": "as11-40-5874"}')
    assert output is not None
    assert "collection" in output
...
def test_get_media_metadata_location() -> None:
    ...
    nasa = NasaAPIWrapper()
    output = nasa.run("get_media_metadata_location", '{"nasa_id": "as11-40-5874"}')
    assert output is not None
    assert "collection" in output
...
def test_get_video_captions_location() -> None:
    ...
    nasa = NasaAPIWrapper()
    output = nasa.run("get_video_captions_location", '{"nasa_id": "172_ISS-Slosh.sr"}')
    assert output is not None
