...
from tests.fake.callbacks import FakeCallbackHandler
...
async def test_async_chat_openai_streaming_with_function() -> None:
    ...
    chain = prompt | chat.bind(
        functions=[function], function_call={"name": function["name"]}
    )
    response = await chain.abatch([{"input": "sally is 13 years old"}])
