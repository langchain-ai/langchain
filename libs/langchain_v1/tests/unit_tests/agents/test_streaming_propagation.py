...
async def test_create_agent_streaming_config_propagation():
    ...
    async for event in agent.astream_events(
        ...
        if event_type == "on_chat_model_stream":
            ...
            print(f"Captured stream event: {event['data']}")

    assert seen_stream_events, "Failed: 'on_chat_model_stream' event not captured. This indicates the config might not have propagated correctly."