...
def test_create_agent_with_name_sets_name_on_ai_message() -> None:
    ...
    assert ai_messages[-1].name == agent_name, f"Expected AIMessage.name to be '{agent_name}', got '{ai_messages[-1].name}'"
...
def test_create_agent_without_name_has_none_name() -> None:
    ...
    ai_messages = [msg for msg in result["messages"] if isinstance(msg, AIMessage)]
    assert len(ai_messages) > 0, "Expected at least one AIMessage in response"
    assert ai_messages[-1].name is None, f"Expected AIMessage.name to be None, got '{ai_messages[-1].name}'"
...
def test_create_agent_name_preserved_across_tool_calls() -> None:
    ...
    for ai_msg in ai_messages:
        assert ai_msg.name == agent_name, f"Expected all AIMessages to have name '{agent_name}', but got '{ai_msg.name}'"
...
def test_create_agent_no_name_parameter_with_tools() -> None:
    ...
    for ai_msg in ai_messages:
        assert ai_msg.name is None, f"Expected AIMessage.name to be None when name parameter is omitted, got '{ai_msg.name}'"