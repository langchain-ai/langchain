...
class ModelRetryMiddleware(AgentMiddleware):
    ...
    def wrap_model_call(
        ...
        Returns:
            `ModelResponse` (the final result).

        Raises:
            Exception: If all retries are exhausted, re-raises the last exception.
        """
        ...
        # Unreachable: loop always returns via handler success or raises exception
        if last_exception is not None:
            raise last_exception
        msg = "Unexpected: retry loop completed without returning or raising"
