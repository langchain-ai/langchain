...
class AgentMiddleware(Generic[StateT, ContextT, ResponseT]):
    ...
    transformers: Sequence[Callable[[tuple[str, ...]], StreamTransformer]] = ()
    """Stream transformer factories registered by the middleware.

    Each entry is a scope-aware factory invoked as `factory(scope)` so subgraph
    mini-muxes receive fresh instances. Factories are merged with the
    `transformers` argument of [`create_agent`][langchain.agents.create_agent]
    at graph compile time, after the built-in transformers and before any
