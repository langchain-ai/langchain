...
class ToolNode(RunnableCallable):
        ...
        *,
        ...
        for tool_ in tools:
            if not isinstance(tool_, BaseTool):
                tool_ = create_tool(tool_, message_version="v1")
    ...
    def _validate_tool_call(self, call: ToolCall) -> Optional[ToolMessage]:
        if (requested_tool := call["name"]) not in self.tools_by_name:
            ...
            return ToolMessage(
                content, name=requested_tool, tool_call_id=cast(str, call["id"]), status="error"
            )
        else:
            return None
    ...
    def _validate_tool_command(
        ...
        messages_update = convert_to_messages_v1(messages_update or {})
...
def tools_condition(
    state: Union[list[MessageV1], dict[str, Any], BaseModel],
    messages_key: str = "messages",
