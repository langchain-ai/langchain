...
_SUPPORTED_PROVIDERS = {
    ...
    "azure_ai": ("langchain-azure-ai.chat_models", "AzureAIChatCompletionsModel"),
...
for provider, (pkg, class_name) in _SUPPORTED_PROVIDERS.items():
    try:
        ...
        if provider == "ollama":
            ...
            try:
                ...
                # skip importing ollama
                continue
        continue
    _CHAT_MODELS[provider] = getattr(module, class_name)
...
def _init_chat_model_helper(
    ...
    **kwargs: Any,
    ...
    if model_provider not in _CHAT_MODELS:
        msg = f"Could not import {pkg} python package. Please install it with `pip install {pkg}`"
        raise ImportError(msg)

    return _CHAT_MODELS[model_provider](model=model, **kwargs)
