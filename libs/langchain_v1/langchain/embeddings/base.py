...
for provider, (pkg, class_name) in _SUPPORTED_PROVIDERS.items():
    try:
        ...
        continue
    _EMBEDDINGS[provider] = getattr(module, class_name)
...
def init_embeddings(
    ...
    **Note:** Must have the integration package corresponding to the model provider
    ...
    if provider not in _EMBEDDINGS:
        pkg = _SUPPORTED_PROVIDERS[provider][0]
        msg = f"Could not import {pkg} python package. Please install it with `pip install {pkg}`"
        raise ImportError(msg)

    return _EMBEDDINGS[provider](model=model_name, **kwargs)
