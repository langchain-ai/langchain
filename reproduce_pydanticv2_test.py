...
class DataSoilDashboardQueryPayloadTimeShift(BaseModel):
    shiftInterval: list[PositiveInt] = Field(
        description="Each element in the array represents a time offset relative to the query timestamp for individual time comparison analysis. If time comparison analysis dose not described, keep it **VOID**.",
        max_length=2,
        default=[],
    )
    timeUnit: Literal["DAY"] = Field(
        default="DAY",
        description="The unit of specific comparison time offset. This is the description about each value of unit: Unit **DAY** represents one day.",
    )


class DataSoilDashboardQueryPayloadQueryParamWhereFilter(BaseModel):
    field: str = Field(
        description="The dimension **CODE** in the selected dimension list that requires enums filtering or pattern filtering."
    )
    operator: Literal["IN", "NI", "LIKE", "NOT_LIKE"] = Field(
        description="Operators for enums filtering or pattern filtering."
    )
    value: list[str] = Field(
        description="If for enums filtering, every element represents th practical enums of the dimension. Otherwise for pattern filtering, only **one** element is required and it represents a wildcard pattern.",
        min_length=1,
    )
    ...
    def field_block(cls, v: str, info: ValidationInfo) -> str:
        if v == "dt":
            raise ValueError(
                "Instruction: The time filtering should be described in 'time' field, not in the 'filters' field."
            )
        return v
    ...
    def value_block(
        ...
        if info.data.get("operator") in {"LIKE", "NOT_LIKE"} and v and len(v) > 1:
            raise ValueError(
                "Instruction: For pattern filtering, the size of 'value' in 'where' must be **ONE**."
            )
        return v
...
class DataSoilDashboardQueryPayloadQueryParamWhere(BaseModel):
    ...
    relation: Literal["AND"] = Field(
        description="Boolean relationships between filters..."
    )

    @field_validator("time")
    def time_format_block(
        cls, v: list[Union[int, str]], info: ValidationInfo
    ) -> list[Union[int, str]]:
        if isinstance(v[0], str) and not re.search(time_pattern, v[0]):
            raise ValueError(
                f"Instruction: the start time of time range must be formatted as **{time_fmt}**"
            )
        if isinstance(v[1], str) and not re.search(time_pattern, v[1]):
            raise ValueError(
                f"Instruction: the end time of time range must be formatted as **{time_fmt}**"
            )
        return v
...
class DataSoilDashboardQueryPayloadQueryParamOrderBy(BaseModel):
    field: str = Field(
        description="The metric **CODE** in the selected metric list that requires metric sorting."
    )
    direction: Literal["ASC", "DESC"] = Field(
        description="Sorting direction for specified metric."
    )
...
class DataSoilDashboardQueryPayloadQueryParamGroupBy(BaseModel):
    field: str = Field(
        description="The dimension **CODE** in the selected dimension list for dimension grouping analysis."
    )
...
class DataSoilDashboardQueryPayload(BaseModel):
    ...
    timeShift: DataSoilDashboardQueryPayloadTimeShift = Field(
        description="Time comparison analysis config.",
        default_factory=DataSoilDashboardQueryPayloadTimeShift,
    )
