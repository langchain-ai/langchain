...
lint lint_package lint_tests: # Run linting on the project.
	...
	git --no-pager grep 'from langchain import' docs cookbook | grep -vE 'from langchain import (hub)' && \
	echo "Error: no importing langchain from root in docs, except for hub" && \
	exit 1 || exit 0
