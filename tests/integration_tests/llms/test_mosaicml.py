...
def test_short_retry_does_not_loop() -> None:
    ...
    llm = MosaicML(inject_instruction_format=True, model_kwargs={"do_sample": False}, retry_sleep=0.1)
    instruction = "Repeat the word foo"
    prompt = llm._transform_prompt(instruction)
    expected_prompt = PROMPT_FOR_GENERATION_FORMAT.format(instruction=instruction)
    assert prompt == expected_prompt

    with pytest.raises(ValueError, match='Error raised by inference API: Rate limit exceeded: 1 per 1 second'):
        output = llm(prompt)
        assert isinstance(output, str)
        output = llm(prompt)
        assert isinstance(output, str)
