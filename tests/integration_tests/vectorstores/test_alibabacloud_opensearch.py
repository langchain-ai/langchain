...
class FakeEmbeddingsWithOsDimension(FakeEmbeddings):
    ...
    def embed_documents(self, embedding_texts: List[str]) -> List[List[float]]:
        ...
        return [
            [float(1.0)] * (OS_TOKEN_COUNT - 1) + [float(i)] for i in range(len(embedding_texts))
        ]
...
settings = AlibabaCloudOpenSearchSettings(
    ...
    field_name_mapping={
        ...
        "embedding": "The embedding field name map of index document，the values must be in float16 multivalue type and separated by commas.",
...
def test_create_alibabacloud_opensearch():
...
def test_alibabacloud_opensearch_with_vector_query() -> None:
    opensearch = create_alibabacloud_opensearch()
    output = opensearch.similarity_search_by_vector("foo", k=1)
    assert output == [Document(page_content="foo", metadata={"metadata": "0"})]

    output = opensearch.similarity_search_by_vector("bar", k=1)
    assert output == [Document(page_content="bar", metadata={"metadata": "1"})]

    output = opensearch.similarity_search_by_vector("baz", k=1)
    assert output == [Document(page_content="baz", metadata={"metadata": "2"})]


def test_alibabacloud_opensearch_with_text_and_meta_query() -> None:
    opensearch = create_alibabacloud_opensearch()
    output = opensearch.similarity_search(query="foo", filter={"metadata": "0"}, k=1)
    assert output == [Document(page_content="foo", metadata={"metadata": "0"})]

    output = opensearch.similarity_search(query="bar", filter={"metadata": "1"}, k=1)
    assert output == [Document(page_content="bar", metadata={"metadata": "1"})]

    output = opensearch.similarity_search(query="baz", filter={"metadata": "2"}, k=1)
    assert output == [Document(page_content="baz", metadata={"metadata": "2"})]

    output = opensearch.similarity_search(query="baz", filter={"metadata": "3"}, k=1)
    assert len(output) == 0


def test_alibabacloud_opensearch_with_text_and_meta_score_query() -> None:
    opensearch = create_alibabacloud_opensearch()
    output = opensearch.similarity_search_with_relevance_scores(query="foo", filter={"metadata": "0"}, k=1)
    assert output == [(Document(page_content="foo", metadata={"metadata": "0"}), 10000.0)]
