...
def test_myscale_search_filter() -> None:
    ...
    output = docsearch.similarity_search("far", k=3)
    print(output)
    assert output == [Document(page_content="far", metadata={"first_letter": "f"})]
    output = docsearch.similarity_search("far", k=3, where_str=f"{docsearch.metadata_column}.first_letter='b'")
    assert output == [Document(page_content="bar", metadata={"first_letter": "b"})]
    docsearch.drop()


def test_myscale_search_filter_with_scores() -> None:
    """Test end to end construction and scored search with metadata filtering."""
    texts = ["far", "bar", "baz"]
    metadatas = [{"first_letter": "{}".format(text[0])} for text in texts]
    config = MyScaleSettings()
    config.table = 'test_myscale_search_filter_with_scores'
    docsearch = MyScale.from_texts(
        texts=texts,
        embedding=FakeEmbeddings(),
        metadatas=metadatas,
    ...
    output = docsearch.similarity_search_with_relevance_scores(
        "far", k=1, where_str=f"{docsearch.metadata_column}.first_letter='b'"
    )
    assert output == [
