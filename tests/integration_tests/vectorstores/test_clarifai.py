...
def test_clarifai_with_from_texts() -> None:
    ...
    APP_ID = "test-lang-2"
    NUMBER_OF_DOCS = 1
    docsearch = Clarifai.from_texts(USER_ID, APP_ID, texts, pat=None, number_of_docs=NUMBER_OF_DOCS)
    time.sleep(2.5)
...
def test_clarifai_with_from_documents() -> None:
    ...
    APP_ID = "test-lang-2"
    NUMBER_OF_DOCS = 1
    docsearch = Clarifai.from_documents(USER_ID, APP_ID, [original_doc], pat=None, number_of_docs=NUMBER_OF_DOCS)
    time.sleep(2.5)
    output = docsearch.similarity_search("foo")
...
def test_clarifai_with_metadatas() -> None:
    ...
    docsearch = Clarifai.from_texts(
        USER_ID, APP_ID, texts, pat=None, number_of_docs=NUMBER_OF_DOCS, metadatas=metadatas
    )
    time.sleep(2.5)
