...
class TestFaunaLoader(unittest.TestCase):
    ...
    def test_fauna_loader(self):
        ...
        for doc in docs:
            assert doc.page_content != ""  # assuming that every document has page_content
            assert "id" in doc.metadata and doc.metadata["id"] != ""  # assuming that every document has 'id'
            assert "ts" in doc.metadata and doc.metadata["ts"] != ""  # assuming that every document has 'ts'
