...
def test_base_tool_inheritance_base_schema() -> None:
    ...
    assert simple_tool.args_schema.schema()["properties"] == expected_args
...
def test_tool_lambda_args_schema() -> None:
    ...
    expected_args = {"tool_input": {"title": "Tool Input", "type": "string"}}
    assert tool.args_schema.schema()["properties"] == expected_args
...
def test_deprecated_multiarg_base_tool_throws_warning() -> None:
    with pytest.warns(DeprecationWarning):

        class ComplicatedTool(BaseTool):
            name: str = "complicated_tool"
            description: str = "A Complicated Tool"

            def _run(self, tool_input: str, other_arg: int) -> str:  # type: ignore
                return f"{tool_input} {other_arg}"

            def _arun(self, tool_input: str) -> Coroutine[Any, Any, str]:
                raise NotImplementedError
