    ...
    return OpenAI(
        ...
        callbacks=[
            PromptLayerCallbackHandler(
                pl_id_callback=pl_id_callback, pl_tags=["OPENAI WORKS!"]
            )
        ],
    )
...
def replicate_llm() -> Replicate:
    return Replicate(
        ...
        callbacks=[
            PromptLayerCallbackHandler(
                pl_id_callback=pl_id_callback, pl_tags=["REPLICATE WORKS!"]
            )
        ],
    )
...
def chat_llm() -> ChatOpenAI:
    return ChatOpenAI(
        ...
        callbacks=[
            PromptLayerCallbackHandler(
                pl_id_callback=pl_id_callback, pl_tags=["OpenAIChat WORKS!"]
            )
        ],
    )
...
def test_openai_llm_generate(openai_llm: OpenAI) -> None:
    ...
    assert all(
        isinstance(response, LLMResult) for response in responses
    ), "All responses should be of type LLMResult"
