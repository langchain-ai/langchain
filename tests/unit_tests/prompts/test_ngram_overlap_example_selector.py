...
from langchain.prompts.example_selector.ngram_overlap import NGramOverlapExampleSelector, ngram_overlap_score
...
def test_selector_threshold_zero(selector: NGramOverlapExampleSelector) -> None:
    """Tests that NGramOverlapExampleSelector excludes examples with no ngram overlap with input, if threshold is set to 0.0."""
    selector.threshold = 0.0
    sentence = "Spot can run."
    output = selector.select_examples({"input": sentence})
    assert output == [EXAMPLES[2], EXAMPLES[0]]

def test_selector_threshold_more_than_one(selector: NGramOverlapExampleSelector) -> None:
    """Tests that NGramOverlapExampleSelector excludes all examples, if threshold is greater than 1.0."""
...
def test_ngram_overlap_score(selector: NGramOverlapExampleSelector) -> None:
    """Tests that ngram_overlap_score returns 0.0 for no ngram overlap, 1.0 for complete ngram overlap, and between 0.0 and 1.0 for some ngram overlap."""
    selector.threshold = 1.0 + 1e-9
