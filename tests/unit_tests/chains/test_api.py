...
from langchain.chains.api.base import RequestsChain, APIChain
from langchain.chains.api.prompt import API_URL_PROMPT, API_RESPONSE_PROMPT
from tests.unit_tests.llms.fake_llm import FakeLLM
from langchain import LLMChain

class FakeRequestsChain(RequestsChain):
    output: str
...
def test_api_data() -> dict:
    ...
    """, "question" : "Search for notes containing langchain", "api_url" : "https://thisapidoesntexist.com/api/notes?q=langchain", "api_response" : json.dumps({"success" : True, "results" : [{"id" : 1, "content" : "Langchain is awesome!"}]}), "api_summary" : "There is 1 note about langchain."}
...
def fake_llm_api_chain(test_api_data : dict) -> APIChain:
    ...
    api_url_query_prompt = API_URL_PROMPT.format(api_docs=TEST_API_DOCS, question=TEST_QUESTION)
    ...
    return APIChain(api_request_chain=api_request_chain, api_answer_chain=api_answer_chain, requests_chain=requests_chain, api_docs=TEST_API_DOCS)
