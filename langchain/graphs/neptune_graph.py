...
class NeptuneGraph:
    ...
    def __init__(
        ...
        if use_https:
            self.summary_url = f'https://{host}:{port}/pg/statistics/summary?mode=detailed'
            self.query_url = f'https://{host}:{port}/openCypher'
        else:
            self.summary_url = f'http://{host}:{port}/pg/statistics/summary?mode=detailed'
        ...
        try:
            ...
            raise ValueError(
                "Could not get schema for Neptune database"
            )
    ...
    def _get_node_properties(self, n_labels, types):
        ...
        for label in n_labels:
            ...
            np = {
                'properties': [
                    {'property': k, 'type': v} for k, v in s
                ],
                'labels': label
            }
