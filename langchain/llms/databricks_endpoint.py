...
class DatabricksEndpoint(LLM):
    ...
    - Cluster driver proxy app (recommended for interactive development).
      ...
      server on the driver node to serve the model at "/" using HTTP POST method with
      JSON input/output.
      Please use a port number between [3000, 8000] and let the server listen to the
      driver IP address or simply `0.0.0.0` instead of localhost only.
    ...
    """A function that transforms (prompt, stop) into a JSON-compatible request that
