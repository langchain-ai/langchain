        ...
        **kwargs: Any,
        ...
        if self.input_schema is not None:
            if (
                ...
                raise ValueError(
                    "Invalid type for prompt in input schema. Must be string or array type (e.g. List, Tuple)"
                )
        else:
        ...
        try:
            ...
            raise ValueError(
                f"Response key {layer} not found in response body: {response_body}"
            )
        except IndexError:
            raise ValueError(
                f"Response index {layer} not found in response body: {response_body}"
            )
