...
class ConvoOutputParser(AgentOutputParser):
    ...
    def parse(self, text: str) -> Union[AgentAction, AgentFinish]:
        try:
            # Attempt to parse the text into a structured format (assumed to be JSON stored as markdown)
            ...
            if "action" in response and "action_input" in response:
                ...
                # If the necessary keys aren't present in the response, raise an exception
                raise OutputParserException(f"Missing 'action' or 'action_input' in LLM output: {text}")
        except Exception as e:
            # If any other exception is raised during parsing, also raise an OutputParserException
