...
class ArizeCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        if response.llm_output and 'token_usage' in response.llm_output:
            ...
            self.prompt_tokens = self.total_tokens = self.completion_tokens = 0  # assign default value
        ...
        for generations in response.generations:
            for generation in generations:
                ...
                columns = ['prediction_ts', 'response', 'prompt', 'response_vector', 'prompt_vector', 'prompt_token', 'completion_token', 'total_token']
                data = [[pred_timestamp, response_text, prompt, response_embedding[0], prompt_embedding[0], self.prompt_tokens, self.total_tokens, self.completion_tokens]]
