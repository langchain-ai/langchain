...
class ArizeCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(
        ...
        # Initialize empty lists to store the prompt/response pairs and other necessary data.
        self.prompt_records = []
        ...
        # Create an embedding generator for generating embeddings from prompts and responses.
        self.generator = EmbeddingGenerator.from_use_case(use_case=UseCases.NLP.SEQUENCE_CLASSIFICATION,
                                                          model_name="distilbert-base-uncased",
                                                          tokenizer_max_length=512,
                                                          batch_size=256)
        
        # Create an Arize client and check if the SPACE_KEY and API_KEY are not set to the default values.
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        # Go through each prompt response pair and generate embeddings as well as timestamp and prediction ids
        for generations in response.generations:
            for generation in generations:
              ...
              embedding_features = {
              "prompt_embedding": Embedding(vector=pd.Series(prompt_embedding[0]),data=prompt),
              "response_embedding": Embedding(vector=pd.Series(response_embedding[0]),data=response),
              }
              tags = {
