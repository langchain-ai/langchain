...
def render_prompt_and_examples(
    ...
    Examples:
    # These have not been tested yet. They were just generated by chatgpt so I don't know if they work yet.
    ```
    ...
    for example in examples:
        ...
        example_response = (
            AIMessage(
                content=output_prompt_template.format(
                    **{output_key: example[output_key] for output_key in output_keys}
                )
            ),
        )
        messages.extend([example_query, example_response])
    messages.append(
        HumanMessage(
            content=input_prompt_template.format(
...
def extract_input_variables(template: str, template_format: str = None) -> list[str]:
    """
    Extract the input variables used in the given string template.

    Args:
        template: The string template to analyze.
        template_type: The type of template used, either 'f-string' or 'jinja'. Infers the template type if not provided.
