...
class DeepLake(VectorStore):
    ...
    try:
        from deeplake.constants import DEFAULT_VECTORSTORE_DEEPLAKE_PATH  # type: ignore
    except ImportError:
        raise ValueError(
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            token (str, optional):  Activeloop token, used for fetching credentials to
                the dataset at path if it is a Deep Lake dataset. This is optional,
                tokens are normally autogenerated.
            embedding_function (Embeddings, optional): function to convert string
                into embedding.
            read_only (bool): whether to open the dataset in read-only mode.
            ingestion_batch_size (int): batch size used during the upload of the
                data to deeplake dataset.
            num_workers (int): number of workers used to upload the data to
                deeplake dataset.
            verbose (str): whether to display the summary of the dataset. Defaults
                to False.
            exec_option (str): Type of query execution. It could be either "python",
                "compute_engine" or "tensor_db". Defaults to "python".
                - ``python`` - Pure-python implementation that runs on the client
                    and can be used for data stored anywhere. WARNING: using this option
                    with big datasets is discouraged because it can lead to memory
                    issues.
                - ``compute_engine`` - C++ implementation of the Deep Lake Compute
                        Engine that runs on the client and can be used for any
                        data stored in or connected to Deep Lake. It cannot be used
                        with in-memory or local data.
                - ``tensor_db`` - Fully-hosted Managed Database that is responsible
                        for storage and query execution. Only available for data
                        stored in the Deep Lake Managed Database. This is achieved
                        by specifying `runtime = {"tensor_db": True}` during dataset
                        creation.
        """
        import deeplake
        ...
        if version > "3.5.2":
            raise ValueError(
                f"deeplake version should be <= 3.5.2, but you've installed {version}. "
                "Consider downgrading deeplake version to 3.5.2 ."
            )
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        """
        from deeplake.util.exceptions import FailedIngestionError  # type: ignore
    ...
    def _search_helper(
        ...
        **kwargs: Any,
        ...
        if use_maximal_marginal_relevance:
            logger.warning(
                "Using maximal marginal relevance with big datasets is discouraged as "
