...
class DeepLake(VectorStore):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            dataset_path (str): Path to existing dataset or where you want to create a new dataset.
                Defaults to _LANGCHAIN_DEFAULT_DEEPLAKE_PATH.
            token (str, optional):  Activeloop token, used for fetching credentials to the dataset 
                at path if it is a Deep Lake dataset. This is optional, tokens are normally autogenerated.
            embedding_function (str, optional): Function that will be used to convert either documents or query.
            read_only (bool): Whether to open dataset in read-only mode. Defaults to False.
            ingestion_batch_size (int): During data ingestion, data is divided into batches.
                The batch size represents the size of each batch. Defaults to 1000.
            num_workers (int): Number of workers to use during data ingestion. Defaults to 0.
            verbose (bool): Whether to print dataset summary after each operation. Defaults to True.
    ...
    def tql_search(self, tql_query, exec_option, return_score=False):
        """Function for performing tql_search
        
        Examples:
        >>> db.tql_search("SELECT * WHERE id == <some_docs_id>, exec_option="compute_engine")
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            lambda_mult: Number between 0 and 1 that determines the degree
                        ...
                        to maximum diversity and 1 to minimum diversity.
                        Defaults to 0.5.
            embedding_function (Optional[Callable]): Embedding function to use. Defaults to None.
            exec_option (str, optional): DeepLakeVectorStore supports 3 ways to perform searching. 
                    It could be either "python", "compute_engine" or "tensor_db". Defaults to "python".
                - ``python`` - Pure-python implementation that runs on the client and can be 
                    used for data stored anywhere. WARNING: using this option with big datasets 
