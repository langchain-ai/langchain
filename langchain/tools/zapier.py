...
class ZapierNLARunAction(BaseTool):
    ...
    Args:
        ...
        instructions: a natural language instruction string for using the action (eg. "grab the latest email from Mike Knoop" for a "Gmail: find email" action)
        params: a dict, strictly optional. Any params provided will *override* AI guesses from `instructions` (see "understanding the AI guessing flow" here: https://nla.zapier.com/api/v1/dynamic/docs)
    """

    name = "Zapier NLA: Run Action"
    description = zapier_nla_base_desc + (
        "This tool will run a specified action and return a stringified JSON result of the API call. "
        "The return result is guarenteed to be less than ~500 words (350 tokens), safe to insert back into another LLM prompt."
    )
