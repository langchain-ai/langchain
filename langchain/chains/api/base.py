...
class APIChain(Chain, BaseModel):
  ...
  @classmethod
  def from_llm_and_api_docs(cls, llm: LLM, api_docs: str):
    get_request_chain = LLMChain(llm=llm, prompt=API_URL_PROMPT)
    requests_wrapper = RequestsChain()
    get_answer_chain = LLMChain(llm=llm, prompt=API_RESPONSE_PROMPT)
    return cls(request_chain=get_request_chain, answer_chain=get_answer_chain, requests_wrapper=requests_wrapper, api_docs=api_docs)