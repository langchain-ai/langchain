...
class Chain(BaseModel, ABC):
    ...
    def __call__(
        ...
        if not isinstance(inputs, dict):
            potential_input_keys = set(self.input_keys).difference(self.memory.memory_variables)
            if len(potential_input_keys) != 1:
                raise ValueError(
                    f"A single string input was passed in, but this chain expects "
                    f"multiple inputs ({potential_input_keys}). When a chain expects "
                    f"multiple inputs, please call it by passing in a dictionary, "
                    "eg `chain({'foo': 1, 'bar': 2})`"
                )
            inputs = {list(potential_input_keys)[0]: inputs}
