...
Install the package with `pip install langchain-serve`.

#### Usage

1. Wrap your LangChain app with the `@serving` decorator. 
...
2. Deploy on Jina AI Cloud with `lc-serve deploy jcloud app`. Once deployed, we can send a POST request to the API endpoint to get a response.

```bash
curl -X 'POST' \
  'https://<your-app>.wolf.jina.ai/ask' \
  -d '{
...
3. You can also self-host the app on your infrastructure with Docker-compose or Kubernetes. See [here](https://github.com/jina-ai/langchain-serve#-self-host-llm-apps-with-docker-compose-or-kubernetes) for more details.
