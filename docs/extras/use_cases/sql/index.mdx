...
- Generating queries that will be ran based on natural language questions.
- Creating chatbots that can answer questions based on database data.
- Building dynamic dashboards based on insights a user wants to analyze.

## Overview
LangChain provides a set of tools for any user to interact with SQL Databases in many ways. 
As an overview, we can:

1. `Build SQL queries` based on user questions to run them separately.
2. Use `SQL Database Chain` for handling both query creation and execution.
3. Use `SQL Agent` that contains the necessary tools for interacting with SQL databases.
...
## Case 1: Using LLM only to create SQL Query.
...
After building the SQL query based on a user question, we can just execute the query:
...
As we can see, the SQL Query Builder chain **only created** the query, and we handled the **query execution separetely**.

### Go deeper
The query builder can be improved in several ways, such as (but not limited to):
- Finetuning database description to your specific use case.
- Including static examples of other questions and their corresponding SQL query in the prompt. 
- Using Vector Databases to include dynamic examples that are relevant to the specific user question.

All these involve customizing the chain's prompt using the following:
...
```
So, with that custom change, if someone asks for the foobar table,
the LLM will understand the user is asking about employees.

```python
chain = create_sql_query_chain(ChatOpenAI(temperature=0), db, prompt=CUSTOM_PROMPT)
chain.run("How many employees are there in the foobar table?")
```
    
    SELECT COUNT(*) FROM Employee

We can also include **few-shot examples** in the prompt, which may be _harcoded question-SQL matches_ that are known to work
correctly, or _dynamic question-SQL matches_ that were retreived before using the chain:
...
## Case 2: Using chain to execute whole process
...
**Be careful** while using this appraoch as it is susceptible to `SQL Injection`. The chain is
...
- [Specifying custom table information](#custon-table-information)
...
1. Determine which tables to use based on the user question.
2. Call the normal SQL database chain using only relevant tables.
...
## Case 3: Using LangChain SQL Agent.

LangChain already provides an Agent which uses the
[SQL Agent Toolkit](https://python.langchain.com/docs/integrations/toolkits/sql_database)
which can be used both for creating and executing queries, as well as using other
tools such as describing a database.
...
query syntax, retreive table descriptions, and more.
