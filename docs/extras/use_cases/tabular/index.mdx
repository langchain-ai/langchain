...
In this page, we will cover SQL use case.
...
We will use an **SQLite connection** with **Chinook database**. Follow [installation steps](https://database.guide/2-sample-databases-sqlite/) and place `Chinhook.db` file in a `notebooks` folder at the root of this repository. Then we can load the database:
...
Note that this **both creates and executes** the query. In the following sections, we will cover the 3 different use cases mentioned in the
use case overview.
...
For example:
- [Loading a CSV file](https://python.langchain.com/docs/integrations/document_loaders/csv)
- [Loading a Pandas DataFrame](https://python.langchain.com/docs/integrations/document_loaders/pandas_dataframe)
Here you can [check full list of Document Loaders](https://python.langchain.com/docs/integrations/document_loaders/)
...
- [Specifying custom table information](https://langchain-git-fork-manuel-soria-master-langchain.vercel.app/docs/use_cases/tabular/sqlite#custom-table-info)
