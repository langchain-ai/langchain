...
Without either, the agent you are trying to build will not work.
If you don't give the access to a correct set of tools, it will never be able to accomplish the correct objective.
If you do not describe the tools properly, the agent will never be able to know how to properly use them.
...
Often times, the group of tools an agent has access to is more important than a single tool.
LangChain provides a concept of toolkits - groups of tools specifically focused on accomplishing a single objective.
These are generally around 3-5 tools in total.
...
The agent executor is the runtime for the agent.
This is what actually calls the agent and executes the actions it suggests.
...
- [Baby AGI](/use_cases/autonomous_agents/baby_agi.html)
- [Auto GPT](/use_cases/autonomous_agents/autogpt.html)
