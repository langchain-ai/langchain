        ...
        if issubclass(cls, BaseLoader):
            for feat in ("aload", "alazy_load", ("lazy_load","lazy_loading")):
                if isinstance(feat, tuple):
                    ...
                    feat, name = feat, feat
                doc_loaders_feat_table[cm][name] = getattr(cls, feat) != getattr(BaseLoader, feat)
            native_async = doc_loaders_feat_table[cm]["aload"] or doc_loaders_feat_table[cm]["alazy_load"]
...
if __name__ == "__main__":
    ...
    document_loader_page = DOCUMENT_LOADER_TEMPLATE.format(table=get_document_loader_table())
