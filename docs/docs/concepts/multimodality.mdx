...
In LangChain, most chat models that support multimodal inputs also accept those values in OpenAI's content blocks format. So far this is restricted to image inputs. For models like Gemini which support video and other bytes cut, the APIs also support the native, model-specific representations.
...
Review the [chat model integration](/docs/integrations/chat/#advanced-features) table for a list of providers that support multimodal inputs.
