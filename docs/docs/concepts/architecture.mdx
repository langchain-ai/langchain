...
Popular integrations have their own packages (e.g. `langchain-openai`, `langchain-anthropic`, `langchain-xai`, etc.) so that they can be properly versioned and appropriately lightweight.
