...
| `reasoning_content`            | Raw              | Usually a string, but can be a list of content blocks. See [content](#content) for details.                                                                                                                             |
...
The **reasoning_content** property of an `AIMessage` represents the thinking generated by the chat model.

The reasoning_content is either:

- **text** -- the norm for virtually all chat models.
- A **list of dictionaries** -- Each dictionary represents a reasoning_content block and is associated with a `type`.
