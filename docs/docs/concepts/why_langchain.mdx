...
1. **Standard component interface:** The growing number of [models](/docs/integrations/chat/) and [related components](/docs/integrations/vectorstores/) for AI applications has resulted in a wide variety of different APIs that developers need to learn and use. 
...
2. **Enhanced component capabilities:** Many components in LangChain support additional features that are particularly useful in AI applications, such as features that mitigate latency and rate limits with many [models](/docs/integrations/chat/)].

3. **Orchestration:** As applications become more complex, combining multiple components and models, there's [a growing need to efficiently connect these elements into control flows](https://lilianweng.github.io/posts/2023-06-23-agent/) that can [accomplish diverse tasks](https://www.sequoiacap.com/article/generative-ais-act-o1/). 
[Orchestration](https://en.wikipedia.org/wiki/Orchestration_(computing)) is crucial for building such applications.

4. **Observability and evaluation:** As applications become more complex, it becomes increasingly difficult to understand what is happening within them. 
...
5. **Deployment:** There's a growing need for seamless transition from prototype to production-ready applications.
...
In the context of [RAG](/docs/concepts/rag/) and LLM application components, LangChain's [retriever](/docs/concepts/retrievers/) interface provides a standard way to connect to many different types of data services or databases (e.g., vector stores or databases).
...
In the case of [retrievers](/docs/concepts/retrievers/), the `invoke` method takes a string and returns a list of [Documents](https://api.python.langchain.com/en/latest/documents/langchain_core.documents.base.Document.html) as output.
...
In addition, to standardizing interfaces, LangChain's component implementations often provide additional features that are particularly useful in AI applications.
As an example, [chat models](/docs/concepts/chat_models/) often suffer from high latency and / or rate limits, which can make them unsuitable for production applications.
LangChain's [chat model](/docs/concepts/chat_models/) implementation has various features that help mitigate these issues:

* **Async:** Many [chat models](/docs/concepts/chat_models/) support [asynchronous execution](/docs/concepts/async), which can help reduce latency by parallelizing requests.
* **Re-tries:** Many [chat models](/docs/concepts/chat_models/) have built-in re-tries, which can help mitigate rate limits.
* **Streaming and batching:** [Chat models](/docs/concepts/chat_models/) support [efficient batching](/docs/concepts/runnables#batch) and [a rich streaming API](/docs/concepts/streaming).
...
As discussed, many individual components in LangChain implement the standard [runnable interface](/docs/concepts/runnables/). Not only does this provides common methods to interact with components, it also serves as a kind of "lego connector": it provides a uniform way to connect components together.
...
LangSmith is our platform that supports observability and evaluation for AI applications. 
...
LangChain and LangGraph are both effective ways to build AI applications. 
But a major pain point for developers is the transition from prototype to production-ready applications.
With this in mind, LangChain offers [LangGraph Cloud](https://langchain-ai.github.io/langgraph/cloud/), a managed service for deploying and hosting LangGraph applications. 
With one click, deploy a production-ready API with built-in persistence for your LangGraph application. 
 LangGraph Cloud APIs are horizontally scalable and deployed with durable storage.
LangGraph Cloud is seamlessly integrated with LangSmith and is accessible from within the LangSmith UI.

## When To Use LangChain?

1. **Flexibility to use different [models](/docs/concepts/chat_models/) and / or associated components:** With the [rapid progress](https://ai.meta.com/blog/llama-3-2-connect-2024-vision-edge-mobile-devices/) in the AI ecosystem, developers have an increasing set of choices in [models](/docs/concepts/chat_models/) and [related components](https://medium.com/@bijit211987/power-of-vector-databases-for-gen-ai-applications-a63d4cf7e352) for AI applications. 
Applications can combine these components in various ways. For example, [RAG](/docs/concepts/rag/) combines [models](/docs/concepts/chat_models/) with [retrieval systems](/docs/concepts/retrieval/)
and [agents](https://langchain-ai.github.io/langgraph/concepts/agentic_concepts/) combine [models](/docs/concepts/chat_models/) with external [tools](/docs/concepts/tool_calling/).
Why use a framework like LangChain when we can just call a particular LLM API directly? 
If you believe that you will only ever want to use a single LLM provider, then it is reasonable to just call their API directly. 
However, if you believe that you will want the flexibility to try out different LLM and / or component providers, then a framework like LangChain can make sense for your application.

2. **Need for low-level control and customization:** Some model providers [offer higher-level abstractions and no/low-code interfaces](https://openai.com/index/introducing-gpts/) for working with [models](/docs/concepts/chat_models/).
These have many benefit. However, if you want to flexibility to experiment with different providers and build custom applications from lower-level components, tailored to specific needs, then a framework like LangChain can make sense for your application.
...
Once you have an application, LangSmith and LangGraph Cloud work seamlessly together to provide observability, evaluation, and deployment for AI applications.