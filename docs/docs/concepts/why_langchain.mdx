...
1. **Proliferation of [models](/docs/concepts/chat_models/) and associated components:** With the [rapid progress](https://ai.meta.com/blog/llama-3-2-connect-2024-vision-edge-mobile-devices/) in the AI ecosystem, we expect developers to have an increasing set of choices for [models](/docs/concepts/chat_models/) and [related components](https://medium.com/@bijit211987/power-of-vector-databases-for-gen-ai-applications-a63d4cf7e352) for AI applications. 
Applications will combine these components in various ways. For example, [RAG](/docs/concepts/rag/) combines [models](/docs/concepts/chat_models/) with [retrieval systems](/docs/concepts/retrieval/)
...
2. **Developer demand for low-level control and customization:** While some model providers [will offer higher-level abstractions and no/low-code interfaces](https://openai.com/index/introducing-gpts/) for working with [models](/docs/concepts/chat_models/), we believe developers still will want to experiment with different provider and component combinations. 
They will want the flexibility to build custom applications from lower-level components, tailored to specific needs.
