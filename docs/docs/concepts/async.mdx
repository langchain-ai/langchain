...
This conceptual guide will not cover the basics of asynchronous programming in Python. There are many resources available online that cover this topic in depth. Here, we will describe how LangChain supports asynchronous programming and how to use it in your applications.

## Why Async Programming?

Asynchronous programming (or async programming) is a paradigm that allows a program to perform multiple tasks concurrently without blocking the execution of other tasks, improving efficiency and responsiveness, particularly in I/O-bound operations (e.g., API calls, file operations). Async programming does not improve performance in CPU-bound operations (e.g., heavy computation).

Many LLM based applications are I/O-bound as they involve making API calls to language models, databases, or other services. As such, async programming is a natural fit for these applications.
...
Please read [Propagation RunnableConfig](/docs/concepts/runnables#propagation-runnableconfig) for more details to learn
how to propagate the `RunnableConfig` down the call chain manually (or upgrade to Python 3.11 where this is no longer an issue).
