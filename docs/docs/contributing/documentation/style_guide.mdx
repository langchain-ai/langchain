...
gain understanding of concepts and how they connect and work together by showing one way to achieve some goal in a hands-on way. It should **avoid** giving
various ways to achieve that same goal in-depth. Instead, it should guide new users along a recommended path to accomplishing the tutorial's goal. While the end result of a tutorial does not necessarily need to
be completely production-ready, it should be useful and practically satisfy the the goals that you clearly stated in the tutorial's introduction. Information on how to address additional scenarios
...
- When a LangChain concept is first mentioned, use its full name (e.g. "LangChain Expression Language (LCEL)"), and link to its conceptual/other documentation page.
...
It should assume that the user is already familiar with underlying concepts and is focused on solving an immediate problem, but
should still provide some background or highlight scenarios where the information is particularly relevant.
It can and should discuss alternatives if one approach may be better than another in certain cases.
...
- Go into detail on the required inputs and outputs
- Brief details when one might use the feature is fine, but in-depth details belong in other sections.

Each category serves a distinct purpose and requires a specific approach to writing and structuring the content.

## General guidelines

Here are some other guidelines you should consider when writing and organizing documentation.
...
Since sections of the docs do not exist in a vacuum, it is important to link to other sections whenever possible
to allow developers to learn more about an unfamiliar topic in detail.

This includes linking to both the API references and conceptual sections!

### Be concise

In general, take a "less-is-more" approach. If a section with good explanation of a concept already exists, you should link to it rather than
re-explaining it, unless the concept you are documenting present new complexities.
