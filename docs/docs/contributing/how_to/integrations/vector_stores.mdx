...
|-------------------------------------------|-------------------------------------------------------------|-------------------|
| `add_documents`                           | Add documents to the vector store.                          | Required          |
| `delete`                                  | Delete selected documents from vector store (by IDs)        | Required          |
| `get_by_ids`                              | Get selected documents from vector store (by IDs)           | Required          |
| `similarity_search                        | Get documents most similar to a query.                      | Required          |
| `embeddings` (property)                   | Embeddings object for vector store.                         | Required          |
| `from_texts`                              | Instantiate vector store via adding texts.                  | Required          |
| `similarity_search_with_score`            | Similarity search, returning score.                         | Optional          |
| `asimilarity_search_with_score`           | Async variant of `similarity_search_with_score`.            | Optional          |
| `similarity_search_by_vector`             | Similarity search from an embedding vector.                 | Optional          |
| `asimilarity_search_by_vector`            | Async variant of `similarity_search_by_vector`.             | Optional          |
| `max_marginal_relevance_search`           | Maximum marginal relevance search.                          | Optional          |
| `amax_marginal_relevance_search`          | Async variant of `max_marginal_relevance_search`.           | Optional          |
| `max_marginal_relevance_search_by_vector` | Maximum marginal relevance search from an embedding vector. | Optional          |
| `amax_marginal_relevance_search`          | Async variant of `max_marginal_relevance_search_by_vector`. | Optional          |
| `add_texts`                               | Add documents to vector store via iterable of strings       | Optional          |
| `aadd_texts`                              | Async variant of `add_texts`.                               | Optional          |
| `aadd_documents`                          | Async variant of `add_documents`.                           | Optional          |
| `adelete`                                 | Async variant of `delete`.                                  | Optional          |
| `aget_by_ids`                             | Async variant of `get_by_ids`.                              | Optional          |
| `from_documents`                          | Instantiate vector store via adding Documents.              | Optional          |

Note that `InMemoryVectorStore` implements some of the optional search types, as well
as convenience methods for loading and dumping the object to a file, but this is not
...
By subclassing the base classes for each type of standard test (see below), you get
all of the standard tests for that type, and you can override the properties that the
test suite uses to configure the tests.
