...
- LLMs: takes a string as input and returns a string
...
- `FunctionMessage`: A `BsaseMessage` coming from a function call.
...
And the `predict_messages` method to run over a list of messages.
...
For both these methods, you can also pass in parameters as runtime arguments.
For example, you could pass in `llm.predict(text, temperature=0)` to adjust the temperature for this invocation.
Runtime arguments always override parameters the object was configured with.
