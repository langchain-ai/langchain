...
Strings are simple, but what exactly are messages? The base message interface defined by `BaseMessage`, which has two required attributes:
...
- Read up on LangSmith, the platform for
