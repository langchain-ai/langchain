...
pages = text_splitter.split_text(docs[0].page_content)
split_docs = [Document(page_content=page, metadata=docs[0].metadata) for page in pages]
vector = FAISS.from_documents(documents=split_docs, embedding=embeddings)
```
