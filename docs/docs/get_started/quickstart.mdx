...
See [here](/docs/integrations/chat) for the list of supported chat models.
...
See [here](/docs/integrations/text_embedding) for the list of supported embedding models.
...
See [here](/docs/integrations/vectorstores) for the list of supported vectorstores.
We provide examples for accessing vectorstores via API or by running local ones.

<Tabs>
  <TabItem value="astradb" label="Astra DB (API)" default>
...
Accessing the API requires an API token and an API endpoint, which you get by creating a free-tier account.
See [here](https://docs.datastax.com/en/astra/astra-db-vector/get-started/quickstart.html#create-a-serverless-vector-database) for the steps to get your API token and API endpoint.
...
vector = AstraDBVectorStore.from_documents(
    documents=documents,
...
```
  ...
  <TabItem value="faiss" label="FAISS (local)">

We will use a local vectorstore, [FAISS](/docs/integrations/vectorstores/faiss).
...
Replace the Astra DB API key and endpoint with your own in the following `server.py` file.
```python
#!/usr/bin/env python
