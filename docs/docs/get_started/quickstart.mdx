...
We've now successfully set up a basic LLM chain. We only touched on the basics of prompts, models, and output parsers - for a deeper dive into everything mentioned here, see [this section of documenation](/docs/modules/model_io).
...
Next, we need to index it into a vectorstore. This includes a few components, namely an [embedding model](/docs/modules/data_connection/text_embedding) and a [vectorstore](/docs/modules/data_connection/vectorstores).
...
We will use a simple local vectorstore for simplicity sake.
...
This chain will take an incoming question, look up relevant documents, then pass those documents along with the orginal question into an LLM and ask it to answer the original question.
...
document_chain.invoke({
    ...
    "context": [Document(page_content="langsmith is a platform")]
})
```

But we don't want to pass in documents directly - instead we want the documents to come from a retriever. We can do this with a separate chain.
...
>> Based on the provided context, LangSmith appears to be a tool or platform developed by LangChain...
```
...
We've now successfully set up a basic retrieval chain. We only touched on the basics of retrieval - for a deeper dive into everything mentioned here, see [this section of documenation](/docs/modules/data_connection).
...
```
You shoul see that this returns documents about testing in LangSmith. This is because the LLM generated a new query, combining the chat history with the follow up question.
...
The search tool that we will use is [Tavily](https://app.tavily.com/). This will require an API key (they have generous free tier). If you do not want to set up an API key, you can skip creating this tool.
...
We've now successfully set up a basic agent. We only touched on the basics of agents - for a deeper dive into everything mentioned here, see [this section of documenation](/docs/modules/agents).
...
remote_chain.invoke({"input": "what is langsmith?"})
```
