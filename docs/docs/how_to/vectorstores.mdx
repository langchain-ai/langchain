...
In the [Pinecone console](https://app.pinecone.io), create a new index with `dimension=1536` called "langchain-test-index". Then, copy the API key and index name. For a more detailed guide to creating a Pinecone index, check out the [Pinecone integration page](/docs/integrations/vectorstores/pinecone).
