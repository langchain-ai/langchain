...
A graph vector store is a special type of vector store, that use both vector similarity and graph connections to retrieve documents.

- [How to: link Documents on common named entities (using GLiNER)](/docs/how_to/graph_vectorstore_link_named_entities)
