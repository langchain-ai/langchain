...
- Full compatibility with [chat models](/docs/concepts/chat_models). Early memory implementations in LangChain weren't designed for the newer chat model APIs, causing issues with features like tool-calling. LangGraph memory can persist any custom state.
