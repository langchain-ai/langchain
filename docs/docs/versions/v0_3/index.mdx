...
Here is a complete list of all packages that have been released and what we recommend upgrading your dependencies to:
Base packages:
...
If you're using `langchain` / `langchain-community` / `langchain-core` / `langserve` 0.2, update your application to use `>=0.3,<0.4` versions of these.

If you're using `langgraph`, upgrade to `langgraph>=0.2.20,<0.3`. This will work with either langchain-core 0.2 or 0.3.

If you're using integration packages (`langchain-x`)
