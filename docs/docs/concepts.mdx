...
LangChain now includes an `ExperimentalMarkdownSyntaxTextSplitter` which is designed to handle Markdown syntax while preserving the original whitespace and formatting. This splitter extracts structured metadata such as headers and code blocks, and allows for customization of headers for splitting. It is particularly useful for maintaining the structure of Markdown documents, including code blocks and horizontal rules.
...
| Markdown  | [MarkdownHeaderTextSplitter](/docs/how_to/markdown_header_metadata_splitter/), [ExperimentalMarkdownSyntaxTextSplitter](/docs/how_to/experimental_markdown_syntax_text_splitter/) | Markdown specific characters                                                                                    | ✅             | Splits text based on Markdown-specific characters. Notably, this adds in relevant information about where that chunk came from (based on the Markdown). The experimental splitter retains original whitespace and formatting, offering more flexibility in further processing. |
