...
They have well-defined schemas that can be passed to a model and generate outputs that can be fed back to the model.
...
The name, description and JSON schema are provided as context to the model.
Given a list of available tools, a model can request that one or more tools be called with specific inputs.

#### Tool inputs

A tool can take arbitrary arguments as input. These arguments should be passed in as a dict.

A tool can also take a ToolCall as input. A ToolCall is what is generated by a tool-calling model. Along with the name of the tool to call and the arguments to that tool, it also contains metadata like a tool call ID that is needed for passing the tool result back to a model.

A tool also has access to the RunnableConfig that's passed into whatever chain the tool is a part of. This allows you to write tool logic that can be parameterized by the config. See the how-to guide for [passing in configs here]().
