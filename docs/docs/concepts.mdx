...
LangChain includes LLM wrappers to interact with various LLM prvoiders (OpenAI, Anthropic, Hugging Face) and local LLMs such as Ollama. Also available is a custom LLM class to implement proprietary LLMs or to use a different wrapper not available in langChain.
...
This represents a message with role "system", which tells the model how to behave. This message serves as the prompt for the LLM to tune the responses for certain applications. Not every model provider supports this.
...
Documents are important to LangChain applications as they standardize the text data and are crucial to using certain components.For example, LangChain community vectorstores can only store and retrieve Document type objects.
LangChain also only uses Document type objects for Embeddings models. It is a assumption that Documents are used for formatting data when using LangChain components.
...
though other formats such as [YAML](/docs/how_to/output_parser_yaml/) can be useful too. Structured output can bridge the gap 
between the flexibility of natural language and the precision required by many software systems and data processing tasks. Specifically, it can take unstructured text
and create structued text that can be used to search databases or CSV files using the structured output created.
Below, we'll discuss a few ways to get structured output from models in LangChain.
