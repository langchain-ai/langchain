...
The `role` describes WHO is saying the message. The standard roles are "user"/"human", "assistant"/"ai", "system", and "tool".
...
This represents a message with role "user"/"human".

#### AIMessage

This represents a message with role "assistant"/"ai". In addition to the `content` property, these messages also have:
...
One of the most effective ways to improve model performance is to give a model examples of what you want it to do. The technique of adding example inputs and expected outputs to a model prompt is known as "few-shot prompting". There's a few things to think about when doing few-shot prompting:
...
The key to a good dataset is that the examples are relevant, clear and informative.
The example inputs should be similar to the types of inputs you'll see at runtime, it should be clear what the correct answer was and why, and they should be examples that aren't obvious.
...
- User feedback: users leave feedback on interactions with the application and positive feedback is automatically added to the dataset of examples,
- LLM feedback: models evaluate interactions with the application and select which interactions they think are good future examples.
...
- Randomly,
- By semantic similarity of the inputs,
...
If we insert our examples into the system prompt as a string, we'll need to make sure it's clear to the model where each example begin and which parts are the input versus output. Different models respond better to different syntax's, like [ChatML](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/chat-markup-language), XML, TypeScript, etc.

If we insert our examples as messages, where each example is represented as a sequence of Human, AI messages, we might want to also assign [names](/docs/concepts/#messages) to our messages like `"example_user"` and `"example_assistant"` to make it clear that these messages correspond to different actors then the latest input message.
