...
### Graph Vector Stores
<span data-heading-keywords="graph,graphs,vectorstore,vectorstores,graph vector store,graph vector stores"></span>

Sometimes embedding models don’t capture all the important relationships between documents.
Graph Vector Stores are an extension to both vector stores and retrievers that allow documents to be explicitly connected to each other.
...
For example, a paragraph of text may be linked to URLs based on the anchor tags in it's content and linked from the URL(s) it is published at.
