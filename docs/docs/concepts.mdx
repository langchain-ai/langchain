...
* See our documentation focused on RAG [here](/docs/tutorials/#working-with-external-knowledge).

:::

RAG is only as good as the retrieved documents’ relevance and and detail. Fortunatly, an emerging set of techiques can be employed to design and improve RAG systems. We've focused on taxonomizing and summarizing many of these techniques (see below figure) and will share some high-level strategic guidance in the following sections.

![](/img/rag_landscape.png)

#### Query Translation

First, consider the user of a RAG system and the input(s) they are likely to pass. Ideally, a RAG system should be able to handle a wide range of inputs, from poorly worded questions to complex multi-part queries.
*Using an LLM to review the raw input and modify it in some way is the central idea behind query translation.* This can be as simple as extracting keywords or as complex as generating multiple sub-questions for a complex concept.
...
* RAG-Fusion: https://youtu.be/77qELPbNgxA?feature=shared
...
Second, consider the data sources available to your RAG system. These could be a mix of structured and unstructured data, or even multiple unstructured data sources. *Using an LLM to review the input and route it to the approriate data source is a simple and effective approach for querying across sources.*

:::tip

See our RAG from Scratch vide on routing: https://youtu.be/pfpIndq7Fi8?feature=shared

And see our documentation on [using an LLM to route queries](/docs/how_to/routing/).

:::

#### Query Construction

Third, consider whether any of your data sources require specific query formats. Many structured databases use SQL, but even vectorstores often have specific syntax for applying keyword filters based upon metadata. *Using an LLM to convert a natual language query into a specific query syntax is a general and powerful approach.*
In particular, [text-to-SQL](/docs/tutorials/sql_qa/), [text-to-Cypher](/docs/tutorials/graph/), and [query analysis](/docs/tutorials/query_analysis/#query-analysis) are useful ways to interact with structured, graph, and vector databses, respectively. 
...
Fouth, consider how the design of your index. A simple and powerful idea is to *decouple the documents that you index for retrieval from the documents that you pass to the LLM for generation.* There is a natural tension here. Indexing frequently uses embedding models with vectorstores, which [compress the semantic information of documents into a fixed-size vectors](https://docs.google.com/presentation/d/1MhsCqZs7wTX6P19TFnA9qRSlxH3u-1-0gWkhBiDG9lQ/edit?usp=sharing).
It is often desirable to index focused and semantically related chunks, which enables efficient retrieval [using a measure of similarity between the input and the chunk](https://cameronrwolfe.substack.com/p/the-basics-of-ai-powered-vector-search?utm_source=profile&utm_medium=reader2).
However, LLMs are increacingly capable of processing thousands or millions of tokens.
...
One approach to address this is simply linking indexed documents (e.g., which are often narrow chunks) to the full documents, often creating a many-to-one mapping between smaller sematically related chunks and the full document that they derive from. This marries the best of both words: perform retrieval using chunks better optimized to your search algorithm, but utilize full documents in generation to provide the LLM with the most context possible.
We have a few different retriever abstractions to support this, including parent document and multi-vector retrievers.
...
Fifth, consider ways to improve the quality of your similarity search itself. Embeddings excel at capturing high-level semantic similarity, but may struggle with more keyword-based queries. Many [vectorstores](https://python.langchain.com/v0.2/docs/integrations/retrievers/pinecone_hybrid_search/) offer built-in [hybrid-search](https://docs.pinecone.io/guides/data/understanding-hybrid-search) to combine keyword and semantic similarity.  
In addition, many vectorstores have [maximal marginal relevance](https://python.langchain.com/v0.1/docs/modules/model_io/prompts/example_selectors/mmr/), which attempts to diversify the results of a search to avoid returning similar documents. Finally, it can be 

:::tip

< Add ColBERT: https://youtu.be/cN6S0Ehm7_8?feature=shared>

:::

#### Post-processing

< Add ReRank - https://python.langchain.com/v0.1/docs/integrations/retrievers/cohere-reranker/ and MRR - RAG-Fusion >

#### Generation

< Add LangGraph self-correction stories: adaptive RAG, self-RAG, corrective-RAG >

For a summary of the techniques and their use cases, see the table below:
