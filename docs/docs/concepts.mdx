...
- If multiple options are supported, you can supply a `method` parameter to toggle which one is used.

You may want or need to use other techiniques if:

- Your LLM does not support tool calling.
- You are working with very complex schemas and the model is having trouble generating outputs that match.
