...
:::caution
Pure text-in/text-out LLMs tend to be older or lower-level. Many new popular models are best used as [chat completion models](/docs/concepts/chat_models),
...
you can transform the tool output but also pass it as an artifact (read more about [`ToolMessage.artifact` here](/docs/concepts/messages/#toolmessage))
...
If you are invoking the tool this way and want to include an [artifact](/docs/concepts/messages/#toolmessage) for the ToolMessage, you will need to have the tool return two things.
...
- Chat models that have explicit [tool-calling APIs](/docs/concepts/tool_calling) will be better at tool calling than non-fine-tuned models.
...
Because this method is part of [LangChain Expression Language](/docs/concepts/lcel),
you can handle formatting differences from different outputs using an [output parser](/docs/concepts/output_parsers) to transform
each yielded chunk.
...
[LLM](/docs/concepts/chat_models) or [chat model](/docs/concepts/chat_models) contained in the component calls
...
Tool calling allows a [chat model](/docs/concepts/chat_models) to respond to a given prompt by generating output that
...
3. Format the result of the tool invocations as [`ToolMessages`](/docs/concepts/messages/#toolmessage).
...
parse the output using an [output parser](/docs/concepts/output_parsers) to convert the raw
...
For models that support it, [tool calling](/docs/concepts/tool_calling) can be very convenient for structured output. It removes the
...
[chat model](/docs/concepts/chat_models) using the `.bind_tools()` method. The model will then generate an `AIMessage` containing
