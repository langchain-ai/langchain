...
With most chat models, a `ToolMessage` can appear in the conversation history following an `AIMessage` with tha requests one or more tool calls (via `tool_calls` field).
