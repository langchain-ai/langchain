...
For some use-cases, such as [retrieval with multiple vectors](/docs/how_to/multi_vector/), having some sort of key-value (KV) storage is helpful.
...
more specific `BaseStore[str, bytes]` instance that stores binary data, also called a `ByteStore`, and should internally take care of
serializing and deserializing. This allows for more modularity, as individual [key-value store implementations](/docs/integrations/stores/)
can just store and retrieve bytes rather than needing to adapt to different types of data.
