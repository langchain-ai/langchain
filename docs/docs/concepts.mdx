...
As the model streams back output, it does so as tokens, which the tokenizer decodes into human-readable text.

#### Callbacks

The lowest level way to stream tokens in LangChain is via the [callbacks](/docs/concepts/#callbacks) system.
This was the first technique introduced in LangChain.
