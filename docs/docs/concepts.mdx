...
This property returns a list of `ToolCall`s. A `ToolCall` is just a dictionary with the following arguments:
...
Once the tools are chosen, you will usually want to invoke them and then pass the results back to the
