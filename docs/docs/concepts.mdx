...
RAG is only as good as the retrieved documents’ relevance and and detail. Fortunately, an emerging set of techniques can be employed to design and improve RAG systems. We've focused on taxonomizing and summarizing many of these techniques (see below figure) and will share some high-level strategic guidance in the following sections.
...
**Using an LLM to review and optionally modify the input is the central idea behind query translation.** This serve as a general buffer, optimizing raw user inputs for your retrieval system. 
...
- [Multi-query](https://youtu.be/JChPi0CRnDY?feature=shared): Re-write the user question from multiple perspectives, retrieve documents for each re-written question, return the unique documents for all queries. 
- [Decomposition](https://youtu.be/h0OPWlEOank?feature=shared): Decompose a question into a set of sub-problems / questions, which can either be solved sequentially (use the answer from first + retrieval to answer the second) or in parallel (consolidate each answer into final answer). 
- [Step-back](https://youtu.be/xn1jEjRyJ2U?feature=shared): First prompt the LLM to ask a generic step-back question about higher-level concepts or principles, and retrieve relevant facts about them. Use this grounding to help answer the user question.
- [HyDE](https://youtu.be/SaDzIVkYqyY?feature=shared): Use an LLM to convert questions into hypothetical documents that answer the question. Use the embedded hypothetical documents to retrieve real documents with the premise that doc-doc similarity search can produce more relevant matches. 
...
See our [documentation](/docs/how_to/routing/) and RAG from Scratch video on [routing](https://youtu.be/pfpIndq7Fi8?feature=shared). Logical routing can use an LLM to reason about the query and choose which datastore is more appropriate. Semantic routing embeds both query and a set of prompts, and chooses the appropriate prompt based upon similarity.  

:::

#### Query Construction

Third, consider whether any of your data sources require specific query formats. Many structured databases use SQL. Vectorstores often have specific syntax for applying keyword filters to document metadata. **Using an LLM to convert a natural language query into a query syntax is a popular and powerful approach.**
In particular, [text-to-SQL](/docs/tutorials/sql_qa/), [text-to-Cypher](/docs/tutorials/graph/), and [query analysis for metadata filters](/docs/tutorials/query_analysis/#query-analysis) are useful ways to interact with structured, graph, and vector database respectively. 
...
Here's a summary of a few popular query construction approaches: 
...
Sixth, consider ways to fiter or rank retrieved documents. This is very useful if you are [combining documents returned from multiple sources](/docs/integrations/retrievers/cohere-reranker/#doing-reranking-with-coherererank), since it can can down-rank less relevant document and / or [compress similar documents](/docs/how_to/contextual_compression/#more-built-in-compressors-filters). 

:::tip

See our RAG from Scratch video on [RAG-Fusion](https://youtu.be/77qELPbNgxA?feature=shared), on approach for post-processing across multiple queries:  Re-write the user question from multiple perspectives, retrieve documents for each re-written question, and combine the ranks of multiple search result lists to produce a single, unified ranking with [Reciprocal Rank Fusion (RRF)](https://towardsdatascience.com/forget-rag-the-future-is-rag-fusion-1147298d8ad1).

:::

Here's a summary of a few popular post-processing approaches: 
...
**Finally, consider ways to build self-correction into your RAG system.** RAG systems can suffer from low quality retrieval (e.g., if a user question is out of the domain for the index) and / or hallucinations in generation. A naive retrieve-generate pipeline has no ability to detect or self-correct from these kids of errors. The concept of "flow engineering" has been introduced in the context of code generation: iteratively build an answer to a code question w/ unit tests to check and self-correct errors. Several works have applied this RAG, such as Self-RAG and Corrective-RAG. In both cases, checks for document relevance, hallucinations, and / or answer quality are performed in the RAG answer generation process.

We've found that graphs are a great way to reliably express logical flow. We've implementd ideas from several recent papers [using LangGraph](https://github.com/langchain-ai/langgraph/tree/main/examples/rag), as shown in the figure below (red - routing, blue - fallback, green - self-correction):
...
See several of our videos focusing on LangGraph for self-corrective rag: 
- https://www.youtube.com/watch?v=-ROS6gfYIts
- https://www.youtube.com/watch?v=E2shqsYwxck 

See our cookbooks on LangGraph for self-corrective rag:
- https://github.com/langchain-ai/langgraph/tree/main/examples/rag

See our LangGraph RAG recipies with partners:
