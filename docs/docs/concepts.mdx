...
Note that the usage of the term "memory" in the context of AI agents should not be confused with the concept of computer memory (like volatile, non-volatile, and persistent memory).
...
Therefore, AI agents need robust memory systems that support multi-modality, offer strong operational performance, and enable agent memory sharing as well as separation, while ensuring [speed, scale, and simplicity](https://learn.microsoft.com/azure/cosmos-db/ai-agents#building-a-robust-ai-agent-memory-system).

Read more about [AI agent](https://learn.microsoft.com/azure/cosmos-db/ai-agents).
