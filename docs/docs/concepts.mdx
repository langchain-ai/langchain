...
A tool can take arbitrary arguments as input. At runtime, these arguments should be passed in as a dict to `tool.invoke({...})`.

Alternatively, a tool can take a ToolCall as input, e.g. `tool.invoke(ToolCall(args={...}, ...))`.
A ToolCall is what is generated by a [tool-calling model](/docs/concepts/#functiontool-calling).
Along with the name of the tool to call and the arguments to that tool, it also contains metadata like a tool call ID, which is needed for passing the tool result back to a model.

A tool also has access to the RunnableConfig that's passed into whatever chain the tool is a part of. This allows you to write tool logic that can be parameterized by the chain config. See the how-to guide for [passing in configs here](/docs/how_to/tool_configure/).

#### Tool outputs

If a tool is called directly with its arguments `tool.invoke({...})` then it will produce an arbitrary output that we assume can be passed to a model as the `ToolMessage.content` field.

If a tool is called with a Toolcall `tool.invoke(ToolCall(args={...}, ...))` then it will produce a `ToolMessage(content=..., ...)` where the tool output has already been assigned to the `ToolMessage.content` field.

A tool can also be defined to include an artifact when invoked with a ToolCall: `tool.invoke(Toolcall(args={...}, ...)) -> ToolMessage(content=..., artifact=..., ...)`.
