...
- **Request callbacks**: Defined in the `invoke` method used for issuing a request. In this case, the callbacks will be used only for that specific request (and all sub-requests that it contains (e.g. a call to an LLMChain triggers a call to a Model, which uses the same handler passed in the `invoke` method).
