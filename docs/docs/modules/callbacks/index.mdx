...
Here is an example of how you can pass a constructor callback:
```python
handler = StdOutCallbackHandler()
llm = OpenAI()
prompt = PromptTemplate.from_template("1 + {number} = ")
...
# Assign the handler to the config
config['callbacks'].append(handler)

# Add constructor callbacks
chain = prompt | LLMChain(llm=llm, callbacks=[on_start, on_end, on_error, handler])
```

In the above code, we pass on_start, on_end, on_error, and handler as a list to the callbacks parameter of the LLMChain constructor. This means that the on_start, on_end, on_error and handler callbacks will be called for all invocations of the LLMChain object.

Keep in mind that constructor callbacks are scoped to the object they are defined on, so the on_start, on_end, on_error and handler callbacks will only be used for the LLMChain object and not for any other objects, such as the OpenAI LLM instance.
...
Here is an example of how you can pass a request callback using the config parameter to the Invoke method: (**Note**: the same approach can be used for the `batch`, `ainvoke`, and `abatch` methods.):
```python
handler = StdOutCallbackHandler()
llm = OpenAI()
prompt = PromptTemplate.from_template("1 + {number} = ")
...
In the above code, the StdOutCallbackHandler is used in conjunction with other custom callbacks (on_start, on_end, and on_error) to provide a comprehensive logging and debugging experience. By combining these callbacks, you can get a detailed view of the execution of your runnable, including when it starts and ends, as well as any errors that may occur during the execution.
