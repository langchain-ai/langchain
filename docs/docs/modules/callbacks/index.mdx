...
LangChain provides a callbacks system that provides a set of hooks to allow you entry into various stages of your application. This is useful for loggging, tracing and monitoring.
...
Let us understand how to use callbacks by examining one of the built-in handlers provided by LangChain - the `StdOutCallbackHandler`, which simply logs all events to `stdout`.
...
chain = prompt | model
chain.invoke({"number": 32}, config = config)
```
