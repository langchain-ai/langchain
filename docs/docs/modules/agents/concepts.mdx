...
This represents an agent action to take.
This contains `tool` and `tool_input` properties,
where `tool` is the tool to be invoked, and `tool_input` is the input to that tool.
...
There is only one required key: `intermediate_steps`.

`intermediate_steps` is a list of (action, tool output) pairs.
This can be typed as (`List[Tuple[AgentAction, Any]]`).
These pairs represent actions previously taken (and corresponding observations) in the **current** agent turn.
