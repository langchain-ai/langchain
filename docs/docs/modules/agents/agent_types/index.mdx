...
Whether or not these agent types support tools with multiple inputs. If a tool only requires a single input, it is generally easier for an LLM to know how to invoke it. Therefor, several earlier agent types aimed at worse models may not support them.
