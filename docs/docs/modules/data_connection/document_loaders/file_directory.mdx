...
We can use the `glob` and `exclude_glob` parameters to control which files to load. Note that here it doesn't load the `.rst` file or the `.html` files.


```python
loader = DirectoryLoader('../', glob="**/*.md", exclude_glob="bak/**/*.*")
```
