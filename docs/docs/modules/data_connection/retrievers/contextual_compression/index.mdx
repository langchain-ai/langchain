...
`ListRerank` uses [zero-shot listwise document reranking](https://arxiv.org/pdf/2305.02156.pdf) and functions similarly to `LLMChainFilter` as a robust but more expensive option.


```python
from langchain.retrievers.document_compressors import ListRerank

_filter = LLMChainFilter.from_llm(llm, top_n=1)
compression_retriever = ContextualCompressionRetriever(base_compressor=_filter, base_retriever=retriever)
