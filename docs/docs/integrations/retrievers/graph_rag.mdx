...
<details>
  ...
  <div>
    ...
    1. Install the following packages:
        ```bash
        pip install -qU langchain-graph-retriever graph_rag_example_helpers
        ```

        * `langchain-graph-retriever` provides the graph traversal retriever.
        * `graph-rag-example-helpers` provides the animal data in an easy to consume format.
