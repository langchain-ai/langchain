...
1. The first step is to create a database with the `lantern` extension installed.
    ...
    Follow the steps at [Lantern Installation Guide](https://docs.lantern.dev/get-started/overview) to install the database and the extension. The docker image is the easiest way to get started.
