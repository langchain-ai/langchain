...
Install the `friendli-client` integration package.

  ```bash
  pip install langchain-comminity friendli-client
  ```

Sign in to [Friendli Suite](https://suite.friendli.ai/) 
to create a Personal Access Token, and set it as the `FRIENDLI_TOKEN` environment.
