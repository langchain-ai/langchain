...
print(res.content)
```
Tool calling with Cohere LLM can eiher be binding tool to the llm as seen above. 
...
```
The ReAct agent is a multi-hop tool calling agent that can be used to call multiple tools in sequence.
