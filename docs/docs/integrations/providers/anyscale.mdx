...
>The hosted `Aviary Explorer` is not available anymore. Visit 
> [Anyscale](https://endpoints.anyscale.com/) to experience models served with `RayLLM`.
>`RayLLM` (formerly known as `Aviary`) is an LLM serving solution that makes it easy to 
> deploy and manage a variety of open source LLMs, built on 
