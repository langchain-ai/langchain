...
- For the smallest installation footprint and to take advantage of features not available in the
  ...
  along with `pip install langchain-unstructured` to use the `UnstructuredSDKFileLoader` or
  `UnstructuredSDKFileIOLoader`. These loaders live in a LangChain partner repo instead of the
...
- To run everything locally, install the open-source python package with `pip install unstructured`
  along with `pip install langchain-community` and use the `UnstructuredFileLoader` or `UnstructuredFileIOLoader`.
