...
>It uses one of the specialized Hugging Face models to detect prompt injection attacks.
> It is useful for when you need to ensure that prompt is free of injection attacks.
> Default is the `laiyer/deberta-v3-base-prompt-injection` model.  
...
See a [usage example](/docs/guides/safety/hugging_face_prompt_injection).
