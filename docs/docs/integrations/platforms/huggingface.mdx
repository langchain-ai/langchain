...
To use the `OpenVINO` backend, please install the `optimum` library 
and set HuggingFacePipeline's backend as `openvino`:
...
pip install transformers
```
See a [usage example](/docs/integrations/chat/huggingface/#huggingfacehub).
...
pip install huggingface_hub
```
See a [usage example](/docs/integrations/chat/huggingface/#huggingfaceendpoint).
...
We need to the API Key to get access to the `Hugging Face Text Generation Inference (TGI)` service.

See a [usage example](/docs/integrations/chat/huggingface/#huggingfacetextgeninference).
